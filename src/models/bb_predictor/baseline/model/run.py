"""
Baseline bb_predictor — walk probability per plate appearance.
Run from src/models/bb_predictor/ with: python baseline/model/run.py
...
expected_role.py already compute pitcher/batter walk_rate by (pre-game-
estimable) role, which is exactly what this target needs (the same
_create_pitcher_pa_outcome_stats / _create_batter_pa_outcome_stats
aggregations k_predictor's strikeout_rate already reuses also compute
walk_rate in the same pass). This script only builds the PA-grain is_walk
label (bb_predictor.processing) and wires three baselines against the
existing rate features:
  1. Naive (most frequent class): predict "no walk" for every PA.
  2. Naive (per expected-pitcher-role walk rate): predict the train-set BB
     rate for that role (sp vs bullpen) — the harder floor, since role alone
     is a real (if coarse) signal.
...
# Same reason as hit_predictor/k_predictor/n_pa_predictor: pitcher_start_depth_stats
...
# knowable estimates, same reasoning as hit_predictor's/k_predictor's own
# experiments.
...
# ------------------------- SEASON-LEVEL WALK RATES ---------------------------- #
# Already computed by hit_predictor — no new feature engineering needed here.
# The same _create_pitcher_pa_outcome_stats / _create_batter_pa_outcome_stats
# aggregations that produce strikeout_rate also produce walk_rate in the same
# pass.
...
print("Evaluating naive (per expected-pitcher-role walk rate)...")
# Falls back to the train-set global walk rate wherever expected_pitcher_role
# is missing — same fallback shape as build_expected_start_innings' baseline
# chain.
...
if not beats_floor:
    ...
    print("  walk_rate, handedness) has no demonstrated signal beyond reading off")
...
if not beats_floor:
    interpretation = (
        ...
        "The current feature set (expected_pitcher_role, season-level pitcher/batter walk_rate, "
        "handedness) has no demonstrated signal beyond a coarse pre-game role split — do not carry "
        "this feature set into a real experiment as-is. Consider rolling-window walk rate (recent "
        "form, already computed by hit_predictor's rolling_stats.py) and times_through_order next."
    )
else:
    best_name, best = max(beats_floor.items(), key=lambda t: t[1]["pr_auc"])
    interpretation = (
