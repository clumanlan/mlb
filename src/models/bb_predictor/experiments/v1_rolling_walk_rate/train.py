"""
bb_predictor experiment v1: rolling walk rate + times_through_order.
Run from src/models/bb_predictor/ with: python experiments/v1_rolling_walk_rate/train.py
...
# season-level pitcher/batter walk_rate + handedness only; logistic
# regression beat naive, PR-AUC 0.0964 vs 0.0757). This experiment adds the
# two features baseline_results.md's own "Next steps" flagged as deferred to
# keep the baseline minimal — both already computed by hit_predictor's
# rolling_stats.py/expected_role.py, zero new feature engineering needed:
#
#   1. ROLLING WALK RATE (this season, recent form): pitcher_roll_season_pa_walk_rate
#      + pitcher_roll_season_pa_total (pitcher side, via
#      rolling_stats.build_pbp_pitcher_rolling_feats_all_roles — same function
#      k_predictor's v1 reused for its own rolling K rate) and
#      batter_roll_season_pa_walk_rate + batter_roll_season_pa_total (batter
#      side, via rolling_stats.build_pbp_batter_rolling_feats — first time
#      this function is wired into any model's train.py; the sample-size
#      denominator pa_total is kept alongside each rate, same "expose the
#      denominator" convention rolling_stats.py documents throughout).
#   2. TIMES_THROUGH_ORDER: expected_times_through_order — already a byproduct
#      of expected_role.assign_expected_pitcher_role (called in the baseline
#      too), just not previously included in FEATURE_COLS.
#
# Deliberately no pitcher-side workload features (WHIP, IP) this pass — K's
# v1 found those redundant with K-rate; walk rate is the more directly
# relevant signal here, same logic. Evaluated the same way as the baseline
# (PR-AUC primary, ROC-AUC secondary) for a direct, apples-to-apples
# comparison, plus the game-grain aggregation check (not yet run for this
# model).
# ---------------------------------------------------------------------------- #
import yaml
...
from models.bb_predictor.utils.mlflow_logging import log_evaluation_to_mlflow, get_git_sha
...
import models.bb_predictor.processing.pipeline as pipeline
...
# Same reason as baseline/hit_predictor/k_predictor: pitcher_start_depth_stats
...
pbp = pipeline.build_pbp_features_walk(pbp, schedule, player_info)

pa_outcome = pipeline.create_pa_outcome_walk(pbp, batter_boxscore, game_info, schedule)
...
# ------------------------- 1. SEASON-LEVEL WALK RATES ------------------------ #
pitcher_role_season_stats = season_stats.build_pbp_pitcher_feats_all_roles(pbp)[
    ["pitcher_key_id", "pitcher_role", "game_season", "pitcher_last_season_pa_walk_rate"]
...
batter_season_stats = season_stats.build_pbp_batter_feats(pbp)[
    ["batter_id", "game_season", "batter_last_season_pa_walk_rate"]
]
...
# ------------------------- 2. ROLLING WALK RATE (THIS SEASON) ---------------- #
# pbp-based, per-game grain — updates every game rather than once a season.
pitcher_pbp_rolling = rolling_stats.build_pbp_pitcher_rolling_feats_all_roles(pbp, window="season")
pitcher_rolling_cols = ["pitcher_roll_season_pa_walk_rate", "pitcher_roll_season_pa_total"]
pa_outcome = pa_outcome.merge(
    pitcher_pbp_rolling.rename(columns={"pitcher_key_id": "expected_pitcher_key_id", "pitcher_role": "expected_pitcher_role"})[
        ["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"] + pitcher_rolling_cols
    ],
...
batter_rolling_cols = ["batter_roll_season_pa_walk_rate", "batter_roll_season_pa_total"]
pa_outcome = pa_outcome.merge(
    batter_pbp_rolling[["gamepk", "batter_id"] + batter_rolling_cols],
    on=["gamepk", "batter_id"], how="left",
)
...
FEATURE_COLS = [
    ...
    "batter_bat_side",
    # 1. season-level
    "pitcher_last_season_pa_walk_rate",
    "batter_last_season_pa_walk_rate",
    # 2. rolling (this season)
    "pitcher_roll_season_pa_walk_rate",
    "pitcher_roll_season_pa_total",
    "batter_roll_season_pa_walk_rate",
    "batter_roll_season_pa_total",
    # 3. times_through_order
...
print(f"Walk rate — train: {train_df[TARGET].mean():.3f}  val: {val_df[TARGET].mean():.3f}")
...
print("Evaluating naive (per expected-pitcher-role BB rate)...")
...
_eval("Naive (per-role BB rate)", y_val, naive_role_pred.to_numpy())
...
role_pr_auc = results["Naive (per-role BB rate)"]["pr_auc"]
best_naive_name, best_naive_pr_auc = max(
    (("Naive (most frequent)", naive_pr_auc), ("Naive (per-role BB rate)", role_pr_auc)),
    key=lambda t: t[1],
)
...
BASELINE_LR_PR_AUC = 0.0964
BASELINE_LR_ROC_AUC = 0.5767

print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v1 (rolling walk rate + times_through_order)")
...
for name, res in results.items():
    delta = f"{res['pr_auc'] - best_naive_pr_auc:+.4f}" if name not in ("Naive (most frequent)", "Naive (per-role BB rate)") else "—"
...
print("\nInterpretation:")
candidates = {n: r for n, r in results.items() if n not in ("Naive (most frequent)", "Naive (per-role BB rate)")}
...
if vs_baseline > 0.005:
    ...
    print("  rolling walk rate / times_through_order add real signal beyond season-level rate alone.")
    ...
    print("  no demonstrated improvement from adding rolling walk rate / times_through_order this pass.")
print("=" * 72)


# ── 7. Game-grain aggregation check ───────────────────────────────────────────
# Rolls PA-grain predictions up to batter-game grain — "will this batter
# walk at least once in this game," the grain a DK BB-prop actually resolves
# on — same approach as hit_predictor's own "1+ hits" check (BENCHMARKS.md
# §4.5) and k_predictor's own "1+ strikeout" check: game_pred_prob =
# 1 - prod(1-p) over that batter's PAs that game, compared against the
# properly-aggregated naive floor (not just a raw uncertainty term). Reused
# directly from hit_predictor's utils/eval.py — it's generic on any 0/1
# target despite "is_hit" naming internally; those are cosmetic leftovers,
# not hit-specific logic.
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ walk\" (same approach as hit_predictor/k_predictor)")
print("=" * 72)
...
print(f"Batter-game walk rate (1+ BB): {game_results['game_is_hit'].mean():.3f}")
...
plot_calibration_curve(
    ...
    {
        "Naive (per-role BB rate)": {"proba": naive_game_results["game_pred_prob"]},
...
ax.set_title(f"is_walk distribution — train ({FIT_SEASONS[0]}–{FIT_SEASONS[-1]})")
ax.set_xlabel("is_walk")
ax.set_ylabel("PAs")
plt.tight_layout()
...
ConfusionMatrixDisplay(cm, display_labels=["No BB", "BB"]).plot(ax=ax, cmap="Blues", colorbar=False)
...
print("\nLogged all runs to MLflow (experiment: bb_predictor).")
