...
# FIXED 2026-09-02 (k_predictor v13's plan, see ROADMAP.md item 6(e) /
# v13_results.md): build_batter_slot_expansion merges batting_order onto
# (gamepk, lineup_position) with NO team-awareness. Every gamepk here has
# TWO starts (home SP, away SP), each needing a DIFFERENT team's 9 batters --
# passing the full unscoped batting_order (as this script always has) let
# roughly half of all synthetic slots collide against the WRONG team (a
# pitcher's own teammates, whom he never actually faces), corrupting
# batter-identity and opp-team features on those slots. Confirmed via a
# single-season diagnostic during v13's build: match rate against a real
# target was 49.9% (an exact home/away-collision signature) before this same
# fix, 99.7% after. Fixed here by team-scoping batting_order per start --
# expand home-team and away-team starters separately (each unambiguous
# against the correctly opposing lineup), then concatenate.
