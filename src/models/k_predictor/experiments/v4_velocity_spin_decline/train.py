"""
k_predictor experiment v4: velocity/spin-rate decline (raw + explicit trend),
isolated from pitch-mix composition.
Run from src/models/k_predictor/ with: python experiments/v4_velocity_spin_decline/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v4 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Extends v3 (trailing-3-start form, opposing-team K-rate volatility, whiff-
# rate/pitch-efficiency; flat vs v2 at both grains, PR-AUC 0.2820 vs 0.2816,
# but season-level whiff rate ranked #7 of 42 — the one real lead from that
# pass). This pass tests a genuinely different (not outcome-derived) signal:
# pitch-level velocity and spin rate, real Statcast measurements rather than
# another rate built from strikeout/at-bat outcomes.
#
#   1. ALL-PITCH RAW LEVELS (season + trailing-3): pitcher_roll_{season,
#      last3g}_stuff_start_speed_mean / _stuff_spin_rate_mean — already
#      computed by build_pbp_pitcher_rolling_feats's "stuff" aggregation, zero
#      new production code, same calls v3 already makes.
#   2. FASTBALL-ONLY RAW LEVELS + PITCH MIX (season + trailing-3), NEW
#      production code this pass. build_pbp_pitcher_rolling_feats's own
#      stuff_start_speed_mean/stuff_spin_rate_mean POOL EVERY PITCH TYPE a
#      pitcher threw that game — caught before this experiment ran: a
#      fastball (~93-97mph) and a breaking/offspeed pitch (~78-88mph) start
#      from very different baselines, so a pitcher throwing more sliders in
#      his last 3 starts than his season average would show a fake
#      "velocity decline" from pure mix shift, zero real arm-strength
#      change. New rolling_stats.build_pitcher_fastball_rolling_feats(_all_roles)
#      (TDD'd, 5 new tests in tests/hit_predictor/test_rolling_stats.py)
#      isolates fastball-only (Four-Seam Fastball + Sinker; Cutter excluded
#      as a hybrid pitch) start_speed/spin_rate, PLUS the pitch-mix level and
#      volatility (fastball_pitch_rate/_std/_max, same mean/std/max-of-per-
#      game-value shape as build_team_strikeout_volatility) — a pitcher's mix
#      changing at all, independent of velocity, may itself carry signal.
#   3. EXPLICIT TREND (ratio + direction), NOT a plain difference, for BOTH
#      the all-pitch and fastball-only velocity/spin pairs. A plain
#      short-minus-season difference is a linear combination of two columns
#      already in the model — logistic regression (the model that wins every
#      classifier built in this project) could already represent that
#      combination internally by choosing coefficients on the two raw levels,
#      so supplying it explicitly adds no new expressiveness. This matches
#      interaction_feats.py's own documented history: an earlier
#      build_trend_diff_features (plain short-season) dominated a Random
#      Forest's feature importance but measurably HURT its held-out metrics —
#      a deterministic linear combination gives a tree nothing new to split
#      on while diluting its random feature sampling. The RATIO
#      (short/season) is different: it's a genuine nonlinearity a linear
#      model cannot construct from raw inputs on its own. Reused directly,
#      zero new production code: interaction_feats.find_rolling_trend_pairs +
#      build_trend_features, already TDD'd (tests/hit_predictor/
#      test_interaction_feats.py), previously exercised only against a
#      Random Forest (hit_predictor's own v3_interaction_feats) and tuned
#      XGBoost (batters_faced_predictor v7) — both tree-based. This is the
#      first time ratio/direction trend features are tested against a linear
#      model anywhere in this repo. Filtered to just the velocity/spin pairs
#      (not every v3 trailing-vs-season pair also present in pa_outcome) so
#      the result stays attributable to this specific hypothesis.
#
# Evaluated the same way as v1/v2/v3: PR-AUC primary vs best naive floor (PA
# grain), then the game-grain aggregation check ("1+ strikeout this game") via
...
pbp_rolling_cols = [
    ...
    # from v3: already-computed rate columns, never selected before.
    "pitcher_roll_season_command_swinging_strike_rate", "pitcher_roll_season_pa_pitch_count_mean",
    # NEW in v4: raw velocity/spin levels, real Statcast "stuff" measurements
    # rather than another outcome-derived rate.
    "pitcher_roll_season_stuff_start_speed_mean", "pitcher_roll_season_stuff_spin_rate_mean",
]
pa_outcome = pa_outcome.merge(
...
pbp_rolling3_cols = [
    ...
    # from v3: trailing-window whiff rate + pitch efficiency.
    ...
    # NEW in v4: raw trailing-3 velocity/spin levels (paired with the season
    # levels above by find_rolling_trend_pairs below).
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_stuff_start_speed_mean",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_stuff_spin_rate_mean",
]
pa_outcome = pa_outcome.merge(
...
# ------------------------- 2c. FASTBALL-ONLY STUFF + PITCH MIX (NEW in v4) --- #
# Isolates true velocity/spin from pitch-MIX composition -- see
# rolling_stats.py's own module note on build_pitcher_fastball_rolling_feats:
# stuff_start_speed_mean/stuff_spin_rate_mean above pool EVERY pitch type a
# pitcher threw that game, so a mix shift (more sliders, fewer fastballs) can
# move that pooled average with zero real change in arm strength. Also adds
# the pitch-mix level + volatility (fastball_pitch_rate/_std/_max) as
# features in their own right -- a pitcher's mix changing at all, independent
# of velocity, may itself carry signal.
_FASTBALL_STAT_COLS = [
    "fastball_start_speed_mean", "fastball_spin_rate_mean",
    "fastball_pitch_rate", "fastball_pitch_rate_std", "fastball_pitch_rate_max",
]
pitcher_fastball_season = rolling_stats.build_pitcher_fastball_rolling_feats_all_roles(pbp, window="season")
pa_outcome = pa_outcome.merge(
    pitcher_fastball_season.rename(columns={"pitcher_key_id": "expected_pitcher_key_id", "pitcher_role": "expected_pitcher_role"})[
        ["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"]
        + [f"pitcher_roll_season_{c}" for c in _FASTBALL_STAT_COLS]
    ],
    on=["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"], how="left",
)
pitcher_fastball_short = rolling_stats.build_pitcher_fastball_rolling_feats_all_roles(pbp, window=SHORT_PITCHER_WINDOW)
pa_outcome = pa_outcome.merge(
    pitcher_fastball_short.rename(columns={"pitcher_key_id": "expected_pitcher_key_id", "pitcher_role": "expected_pitcher_role"})[
        ["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"]
        + [f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_{c}" for c in _FASTBALL_STAT_COLS]
    ],
    on=["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"], how="left",
)

# ------------------------- 2d. VELOCITY/SPIN TREND (NEW in v4) --------------- #
# Ratio + direction between the trailing-3 and season levels above (both the
# all-pitch-pooled and fastball-only versions) — see this file's own module
# docstring (top of file) for why ratio (not a plain difference) is the right
# transform for a linear model. Filtered to just the velocity/spin stat pairs
# (find_rolling_trend_pairs would otherwise also match every other season/
# trailing-3 pair already in pa_outcome from v3 — K-rate, WHIP, etc.) so this
# result stays attributable to this specific hypothesis, not a broader retest
# of v3's already-flat trailing features.
_VELOCITY_SPIN_STATS = (
    "stuff_start_speed_mean", "stuff_spin_rate_mean",
    "fastball_start_speed_mean", "fastball_spin_rate_mean",
)
_all_trend_pairs = interaction_feats.find_rolling_trend_pairs(pa_outcome.columns.tolist())
_velocity_spin_pairs = [
    (short_col, season_col) for short_col, season_col in _all_trend_pairs
    if short_col.endswith(_VELOCITY_SPIN_STATS)
]
pa_outcome = interaction_feats.build_trend_features(pa_outcome, _velocity_spin_pairs)

# ------------------------- 2e. OPPOSING-TEAM K-RATE VOLATILITY (from v3) ----- #
...
FEATURE_COLS = [
    ...
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_command_swinging_strike_rate",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_pa_pitch_count_mean",
    # 2f. velocity/spin (all-pitch, pooled): raw levels (season + trailing-3)
    # + explicit ratio/direction trend, NEW in v4
    "pitcher_roll_season_stuff_start_speed_mean",
    "pitcher_roll_season_stuff_spin_rate_mean",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_stuff_start_speed_mean",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_stuff_spin_rate_mean",
    "pitcher_trend_ratio_stuff_start_speed_mean",
    "pitcher_trend_direction_stuff_start_speed_mean",
    "pitcher_trend_ratio_stuff_spin_rate_mean",
    "pitcher_trend_direction_stuff_spin_rate_mean",
    # 2g. fastball-only velocity/spin + pitch mix (level + volatility): raw
    # levels (season + trailing-3) + explicit ratio/direction trend for the
    # velocity/spin pair, NEW in v4 — isolates arm-strength signal from
    # pitch-mix composition (see rolling_stats.build_pitcher_fastball_rolling_feats)
    "pitcher_roll_season_fastball_start_speed_mean",
    "pitcher_roll_season_fastball_spin_rate_mean",
    "pitcher_roll_season_fastball_pitch_rate",
    "pitcher_roll_season_fastball_pitch_rate_std",
    "pitcher_roll_season_fastball_pitch_rate_max",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_fastball_start_speed_mean",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_fastball_spin_rate_mean",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_fastball_pitch_rate",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_fastball_pitch_rate_std",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_fastball_pitch_rate_max",
    "pitcher_trend_ratio_fastball_start_speed_mean",
    "pitcher_trend_direction_fastball_start_speed_mean",
    "pitcher_trend_ratio_fastball_spin_rate_mean",
    "pitcher_trend_direction_fastball_spin_rate_mean",
    # 3. rolling shrunk to last season
    "pitcher_shrunk_whip",
...
print(f"Feature count: {len(FEATURE_COLS)} (v2 had 22, v3 had 42)")
...
# v1/v2/v3's own results, hardcoded here for a direct comparison printout —
# not re-derived, since each used a smaller, frozen feature set.
BASELINE_LR_PR_AUC = 0.2702
BASELINE_LR_ROC_AUC = 0.5821
V1_LR_PR_AUC = 0.2732
V1_LR_ROC_AUC = 0.5869
V2_LR_PR_AUC = 0.2816
V2_LR_ROC_AUC = 0.5981
V3_LR_PR_AUC = 0.2820
V3_LR_ROC_AUC = 0.5977

print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v4 (velocity/spin-rate decline: raw + explicit trend)")
...
print(f"{'(v2 LR, for reference)':<28} {V2_LR_PR_AUC:>8.4f} {'':>14}  {V2_LR_ROC_AUC:>8.4f}")
print(f"{'(v3 LR, for reference)':<28} {V3_LR_PR_AUC:>8.4f} {'':>14}  {V3_LR_ROC_AUC:>8.4f}")
print("=" * 72)
...
if vs_v3 > 0.005:
    print(f"  vs v3's logistic regression (PR-AUC {V3_LR_PR_AUC:.4f}): {vs_v3:+.4f} —")
    print("  velocity/spin decline adds real PA-grain signal beyond v3.")
elif vs_v3 < -0.005:
    print(f"  vs v3's logistic regression (PR-AUC {V3_LR_PR_AUC:.4f}): {vs_v3:+.4f} —")
    print("  worse than v3 — check for a wiring bug before concluding the new features don't help.")
else:
    print(f"  vs v3's logistic regression (PR-AUC {V3_LR_PR_AUC:.4f}): {vs_v3:+.4f} — flat,")
    print("  no demonstrated PA-grain improvement from this pass's features.")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1/v2/v3)")
print("=" * 72)
...
# v3's own game-grain result, hardcoded for reference — reliability 0.0002,
# resolution 0.0133, verdict real_improvement.
V3_GAME_RELIABILITY = 0.0002
V3_GAME_RESOLUTION = 0.0133
print(f"\n  (v3's game-grain result, for reference: reliability {V3_GAME_RELIABILITY:.4f}, "
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v4_velocity_spin_decline",
            "target": TARGET,
