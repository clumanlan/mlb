"""
k_predictor experiment v7: opposing-lineup toughest-out (shrunk K-rate MIN)
and best-batter star-power (shrunk OBP/SLG MAX), on top of v6's tuned
XGBoost feature set.
Run from src/models/k_predictor/ with: python experiments/v7_toughest_out_and_star_power/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v7 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Two new opposing-lineup features, both built on a shared "shrink each
# starting-lineup batter's own stat toward last season, then take a
# cross-sectional extremum across the lineup" pattern (batter_workload.py,
# new this session, direct port of pitcher_workload.build_pitcher_shrunk_whip's
# shrinkage recipe):
#
#   1. PRIMARY: opp_team_toughest_out_shrunk_k_rate = MIN(batter_shrunk_k_rate)
#      across the opposing starting lineup. A single elite-contact/low-K
#      batter caps a pitcher's total-K ceiling regardless of how weak the
#      rest of the lineup is -- a team average (the existing
#      opp_team_roll_season_pa_strikeout_rate from v2) hides this entirely.
#   2. EXPLORATORY: opp_team_best_batter_shrunk_obp / _slg =
#      MAX(shrunk OBP) / MAX(shrunk SLG) across the same lineup, independently
#      (may be different players). Built to see what it looks like, per user
#      request -- OBP/SLG measure overall offensive value, not the
#      bat-to-ball skill a strikeout is the direct inverse of, so this is a
#      weaker-motivated mechanism than #1 for this specific target.
#
# Both features are read pre-game off the REALIZED starting lineup (same
# stand-in for real pregame lineup data every other team-pooling feature in
# this file already uses -- not yet ingested anywhere in this repo).
#
...
# re-grid-searching -- same "carry tuned hyperparameters forward unchanged"
# convention batters_faced_predictor's v2-v7 already established. Evaluated
# the same way as v1-v6: PR-AUC primary vs best naive floor (PA grain), then
# the game-grain aggregation check, feature importance read off for the three
# new columns specifically.
# ---------------------------------------------------------------------------- #
import yaml
...
# ------------------------- 4. NEW in v7: TOUGHEST-OUT + STAR-POWER ----------- #
...
FEATURE_COLS = [
    ...
    # 5. NEW in v7 — toughest-out (primary) + best-batter star power (exploratory)
...
NEW_V7_COLS = [
    "opp_team_toughest_out_shrunk_k_rate", "opp_team_best_batter_shrunk_obp", "opp_team_best_batter_shrunk_slg",
]
for c in NEW_V7_COLS:
...
print(f"Feature count: {len(FEATURE_COLS)} (v6's 42 + 3 new v7 columns)")
...
# v6's own results, hardcoded here for a direct comparison printout.
V6_XGB_PR_AUC = 0.2838
V6_XGB_ROC_AUC = None  # not recorded in v6's summary comment — omitted from printout below

print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v7 (toughest-out + best-batter star power)")
...
print("-" * 72)
print(f"{'(v6 XGBoost tuned, for reference)':<28} {V6_XGB_PR_AUC:>8.4f}")
print("=" * 72)
...
vs_v6 = best["pr_auc"] - V6_XGB_PR_AUC
if vs_v6 > 0.005:
    print(f"  vs v6's tuned XGBoost (PR-AUC {V6_XGB_PR_AUC:.4f}): {vs_v6:+.4f} — real gain from the new features.")
elif vs_v6 < -0.005:
    print(f"  vs v6's tuned XGBoost (PR-AUC {V6_XGB_PR_AUC:.4f}): {vs_v6:+.4f} — worse, check for a wiring bug.")
else:
    print(f"  vs v6's tuned XGBoost (PR-AUC {V6_XGB_PR_AUC:.4f}): {vs_v6:+.4f} — flat, same PA-grain-metric pattern as most single-pass feature additions in this project.")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1-v6)")
print("=" * 72)
...
V6_GAME_RELIABILITY = 0.0001
V6_GAME_RESOLUTION = 0.0137
print(f"\n  (v6's game-grain result, for reference: reliability {V6_GAME_RELIABILITY:.4f}, "
      f"resolution {V6_GAME_RESOLUTION:.4f}, verdict real_improvement)")
print("=" * 72)
...
colors = ["darkorange" if name in NEW_V7_COLS else "steelblue" for name in importances.index]
ax.barh(importances.index, importances.values, color=colors)
ax.set_title("Feature importance — XGBoost (v7 new features in orange)")
...
print("\nv7 new-feature ranks (of {} features):".format(len(importances)))
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v7_toughest_out_and_star_power",
            "target": TARGET,
