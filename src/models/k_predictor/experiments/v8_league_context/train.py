"""
k_predictor experiment v8: league-wide rolling context (does the model know
what season/era it's in?), on top of v7's tuned XGBoost feature set.
Run from src/models/k_predictor/ with: python experiments/v8_league_context/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v8 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# ROADMAP.md's 2026-08-29 entry flagged a real, untried gap: every league-wide
# table in this repo before this session was either (a) a STATIC last-season
# snapshot (season_stats.build_league_pa_outcome_stats, never imported by
# k_predictor in any prior version) or (b) scoped to one team's own recent
# form (every opposing-lineup feature v2-v7 already use). Nothing captures
# "how has the game itself shifted this year" -- rule changes (2023's pitch-
# clock/shift-ban/bigger-bases), juiced/dead-ball eras -- as a rolling,
# in-season-updating signal.
#
# Two new production functions (rolling_stats.py, TDD'd this session -- see
# implementation_plan.md), both pooled across the ENTIRE league (every team,
# every batter, no starting-lineup filter -- league context isn't a matchup
# concept the way opposing-lineup features are):
#
#   1. build_league_pa_outcome_rolling_feats -- rolling K/BB/HBP/single/XBH/HR
#      rate, the rolling equivalent of build_league_pa_outcome_stats. Directly
#      on-target: K/BB/HBP are what pitchers allow league-wide, single/XBH/HR
#      are what the ball in play produces league-wide.
#   2. build_league_batter_rolling_stats -- rolling BA/OBP/SLG/ISO/BABIP, the
#      rolling equivalent of build_batter_rolling_stats, pooled to the whole
#      league -- the slash-line/sabermetric half of "what does the league
#      look like right now."
#
# Both are keyed by (game_season, game_date) ONLY -- no gamepk/team_id/
# batter_id, unlike every other feature table in this file. There's no single
# per-game "entity" to roll league-wide (many games share a date), so both
# functions pool to one row per date first, roll with a constant entity
# column (shift(1) at the date grain -- no same-day leakage), and the result
# is merged onto pa_outcome via a plain two-column merge (every gamepk
# sharing a date gets an identical value by construction, so no fan-out risk).
#
# Deliberately NOT attempted this pass (see implementation_plan.md's design
# notes): a starting-lineup-only pooling variant (rejected -- league context
# isn't matchup-specific), a third league-wide pitcher-composite table
# (WHIP/FIP -- needs a new innings-pitched aggregation path, flagged as a
# future pass if this one shows signal), and ROADMAP's separate gap #2
# (opposing-team last-season shrinkage -- kept as its own follow-on so a flat
# result here isn't muddied by an unrelated feature family).
#
...
# re-grid-searching, same convention as v7. Evaluated the same way as v1-v7:
# PR-AUC primary vs best naive floor (PA grain), then the game-grain
# aggregation check, feature importance read off for all 11 new columns
# specifically -- the whole point of this experiment is which half of the
# league signal (PA-outcome rates or slash-line stats), if either, carries
# any weight at all.
# ---------------------------------------------------------------------------- #
import yaml
...
# ------------------------- 5. NEW in v8: LEAGUE-WIDE ROLLING CONTEXT -------- #
...
print(f"Feature count: {len(FEATURE_COLS)} (v7's 45 + 11 new v8 columns)")
...
# v6's tuned XGBoost remains the standing best model after v7's flat result
# (v7 PR-AUC 0.2836, vs v6's 0.2838) — hardcoded here for a direct comparison
# printout, same convention as every prior version's script.
STANDING_BEST_XGB_PR_AUC = 0.2838
STANDING_BEST_GAME_RELIABILITY = 0.0001
STANDING_BEST_GAME_RESOLUTION = 0.0137

print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v8 (league-wide rolling context)")
...
if vs_standing > 0.005:
    print(f"  vs v6's tuned XGBoost (PR-AUC {STANDING_BEST_XGB_PR_AUC:.4f}): {vs_standing:+.4f} — real gain from the league-context features.")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1-v7)")
print("=" * 72)
...
colors = ["darkorange" if name in NEW_V8_COLS else "steelblue" for name in importances.index]
ax.barh(importances.index, importances.values, color=colors)
ax.set_title("Feature importance — XGBoost (v8 new features in orange)")
...
print("\nv8 new-feature ranks (of {} features):".format(len(importances)))
