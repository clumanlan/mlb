"""
k_predictor experiment v10: count-leverage/put-away rate + park-specific
strikeout tendency -- on top of v9's player-vs-league feature set.
Run from src/models/k_predictor/ with: python experiments/v10_count_leverage_park_k/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v10 SUMMARY                               #
# ---------------------------------------------------------------------------- #
# Two independent new feature threads, both closing gaps confirmed absent by
# direct code inspection before this pass (see implementation_plan.md):
#
#   1. COUNT-LEVERAGE / PUT-AWAY RATE. Every prior version's PA-outcome
#      features (pa_strikeout_rate, pa_full_count_rate, etc.) describe
#      overall PA outcomes or the FINAL count reached, not the leverage
#      point of "did this PA ever get to 2 strikes, and what happened once
#      it did." Two new TDD'd columns per pitcher/batter rolling table
#      (rolling_stats.py's _pitcher_pa_outcome_per_game /
#      _batter_pa_outcome_per_game, build_pbp_pitcher_rolling_feats /
#      build_pbp_batter_rolling_feats):
#        - pa_two_strike_reach_rate = PAs that ever reach 2 strikes (MAX
#          count_strikes over every pitch in the PA, not just the final
#          pitch's count -- a foul-off at 2 strikes keeps count_strikes at 2
#          on every subsequent pitch) / total PAs.
#        - pa_put_away_rate = strikeouts / PAs that reached 2 strikes -- a
#          conditional conversion rate, genuinely different information from
#          pa_strikeout_rate (unconditional) even though both are built from
#          the same underlying counts.
#      Landed in hit_predictor's generic rolling_stats.py (not a k_predictor-
#      local file) since pa_strikeout_n/pa_full_count_n already establish
#      this exact category of PA-outcome counting as target-agnostic
#      infrastructure, not model-specific logic. Built for BOTH pitcher and
#      batter sides (per user's request) -- the pitcher's own put-away rate
#      is the more directly relevant signal for THIS model's target, but the
#      opposing batter's own two-strike behavior is independent information
#      the model has never seen.
#
#   2. PARK-SPECIFIC STRIKEOUT TENDENCY. park_factors.py had exactly one
#      function, build_park_factors -- a hit-rate index (H/AB by venue/
#      season vs league), reused as-is by every model in this repo including
#      k_predictor. Confirmed by reading the file: no strikeout-rate
#      equivalent existed. New build_park_strikeout_factor mirrors
#      build_park_factors' exact shape (season-level, _shift_to_last_season,
#      keyed on (venue_id, game_season)) but aggregates pitcher_boxscore's
#      k/ip instead of batter_boxscore's h/ab -- matching season_stats.py's
#      own k_rate = k/ip convention rather than inventing a new denominator.
#
# Both threads land only at window='season' this pass -- consistent with how
# v8's league-wide context and v9's slash-line merge were both introduced
# season-only first, with a trailing-N-game follow-up left as a separate,
# independent decision if the season-window version shows signal.
#
...
# re-grid-searching, same convention as v7/v8/v9. Evaluated the same way as
# v1-v9: PR-AUC primary vs best naive floor (PA grain), then the game-grain
# aggregation check, feature importance read off for the 5 new columns
# specifically (2 pitcher, 2 batter, 1 park) since the two threads are
# independent and should be judged independently even though they share one
# experiment version.
# ---------------------------------------------------------------------------- #
import yaml
...
] + NEW_V9_RAW_COLS + NEW_V9_DERIVED_COLS + NEW_V10_RAW_COLS
...
print(f"Feature count: {len(FEATURE_COLS)} (v9's {56 + len(NEW_V9_RAW_COLS) + len(NEW_V9_DERIVED_COLS)} + {len(NEW_V10_RAW_COLS)} new v10 columns)")
...
for c in NEW_V9_DERIVED_COLS:
    print(f"  {c}: {model_df[c].notna().mean():.1%} non-null, mean {model_df[c].mean():.4f}")
print("New v10 columns (count-leverage/put-away rate + park strikeout tendency):")
for c in NEW_V10_RAW_COLS:
    print(f"  {c}: {model_df[c].notna().mean():.1%} non-null, mean {model_df[c].mean():.4f}")
...
print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v10 (count-leverage/put-away + park strikeout tendency)")
...
if vs_standing > 0.005:
    print(f"  vs v6's tuned XGBoost (PR-AUC {STANDING_BEST_XGB_PR_AUC:.4f}): {vs_standing:+.4f} — real gain from the count-leverage/park-K-tendency features.")
...
colors = [
    "forestgreen" if name in NEW_V10_RAW_COLS
...
ax.set_title("Feature importance — XGBoost (v10 new in green, v9 raw in orange, v9 derived in red)")
...
for c in NEW_V9_DERIVED_COLS:
    rank = list(rank_order.index).index(c) + 1
    print(f"  {c}: rank #{rank}, importance {rank_order[c]:.4f}")
print("\nv10 new column ranks (of {} features):".format(len(importances)))
for c in NEW_V10_RAW_COLS:
    rank = list(rank_order.index).index(c) + 1
    print(f"  {c}: rank #{rank}, importance {rank_order[c]:.4f}")
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v10_count_leverage_park_k",
            "target": TARGET,
