"""
k_predictor experiment v5: fastball-only velocity/spin, stripped down to just
raw levels + direction trend.
Run from src/models/k_predictor/ with: python experiments/v5_fastball_velocity_direction/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v5 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Extends v3 (the last "curated" baseline — trailing-3-start form, opposing-
# team K-rate volatility, whiff-rate/pitch-efficiency; PR-AUC 0.2820, game-
# grain real_improvement, resolution 0.0133). v4 (a superset of v3 plus
# all-pitch-pooled AND fastball-only velocity/spin raw+ratio+direction, plus
# pitch-mix level/volatility — 64 features total) found the single strongest
# individual feature lead in this session's whole residual-driven hunt:
# pitcher_trend_direction_fastball_start_speed_mean ranked #3 of 64, but v4's
# aggregate PA-grain/game-grain deltas vs v3 (+0.0016 / +0.0003) still landed
# below this project's ~0.005 "real" threshold — 22 new columns (many of them
# flat: pitch-mix rate/volatility, the all-pitch-pooled versions, the ratio
# transforms) may have diluted a genuinely strong signal rather than the
# signal itself being too weak. This pass tests that directly: keep ONLY
# v3's 42 features plus the two features that actually mattered in v4 —
# fastball-only velocity/spin raw levels (season + trailing-3) and their
# DIRECTION trend (not ratio, which underperformed direction by a wide margin
# in v4's own importance ranking) — 6 new columns total, not 22.
#
# Also updates rolling_stats.FASTBALL_PITCH_TYPES to add Cutter, per a
# quick 2024 pitch-mix audit this session: of 623 qualified pitchers, 27.4%
# have a non-fastball primary (most-thrown) pitch, and Cutter is the single
# largest such group (37 of 171) — bigger than any individual breaking-ball
# type. Cutter was originally excluded as a "hybrid" pitch on a judgment call
# without data; velocity-wise it sits close to a four-seamer, not a breaking
# ball, so excluding it was probably wrong. The remaining ~21.5% whose
# primary pitch is a genuine breaking/offspeed pitch is a real, separate gap
# a fixed pitch-type bucket can't close — flagged as a follow-up (a pitcher's
# own actual primary pitch, whatever type, would need a season-level
# categorical assignment step), not built this pass.
#
# Evaluated the same way as v1-v4: PR-AUC primary vs best naive floor (PA
...
# ------------------------- 2c. FASTBALL-ONLY VELOCITY/SPIN (NEW in v5) ------- #
# Raw levels only (season + trailing-3) -- deliberately NOT pulling
# fastball_pitch_rate/_std/_max (pitch-mix level/volatility) this pass; v4
# found those flat/unranked, so this pass isolates just the two columns that
# actually mattered.
_FASTBALL_VELOCITY_SPIN_COLS = ["fastball_start_speed_mean", "fastball_spin_rate_mean"]
...
pa_outcome = pa_outcome.merge(
    pitcher_fastball_season.rename(columns={"pitcher_key_id": "expected_pitcher_key_id", "pitcher_role": "expected_pitcher_role"})[
        ...
        + [f"pitcher_roll_season_{c}" for c in _FASTBALL_VELOCITY_SPIN_COLS]
    ],
    on=["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"], how="left",
)
...
pa_outcome = pa_outcome.merge(
    pitcher_fastball_short.rename(columns={"pitcher_key_id": "expected_pitcher_key_id", "pitcher_role": "expected_pitcher_role"})[
        ["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"]
        + [f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_{c}" for c in _FASTBALL_VELOCITY_SPIN_COLS]
    ],
    on=["gamepk", "expected_pitcher_key_id", "expected_pitcher_role"], how="left",
)

# ------------------------- 2d. FASTBALL VELOCITY/SPIN DIRECTION (NEW in v5) -- #
# Direction only (sign of trailing-3 vs season) -- NOT ratio, which v4's own
# importance ranking showed underperforming direction by a wide margin
# (~#24 vs #3 for start_speed) for the same underlying stat. Filtered to just
# the two fastball velocity/spin pairs so this stays a clean, minimal test.
...
_fastball_pairs = [
    ...
    if short_col.endswith(tuple(_FASTBALL_VELOCITY_SPIN_COLS))
]
pa_outcome = interaction_feats.build_trend_features(pa_outcome, _fastball_pairs)
_DIRECTION_COLS_TO_DROP = [c for c in pa_outcome.columns if c.startswith("pitcher_trend_ratio_fastball_")]
pa_outcome = pa_outcome.drop(columns=_DIRECTION_COLS_TO_DROP)
...
FEATURE_COLS = [
    ...
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_command_swinging_strike_rate",
    f"pitcher_roll_last{SHORT_PITCHER_WINDOW}g_pa_pitch_count_mean",
    # 2f. fastball-only velocity/spin: raw levels (season + trailing-3) +
    # direction trend only (not ratio), NEW in v5 -- stripped down from v4's
    # 22 new columns to just the 6 that actually mattered
...
print(f"Feature count: {len(FEATURE_COLS)} (v2 had 22, v3 had 42, v4 had 64)")
...
# v1/v2/v3/v4's own results, hardcoded here for a direct comparison printout
# — not re-derived, since each used a smaller/different feature set.
...
V4_LR_PR_AUC = 0.2836
V4_LR_ROC_AUC = 0.5989

print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v5 (fastball velocity/spin: raw + direction only)")
...
print(f"{'(v4 LR, for reference)':<28} {V4_LR_PR_AUC:>8.4f} {'':>14}  {V4_LR_ROC_AUC:>8.4f}")
print("=" * 72)
...
vs_v4 = best["pr_auc"] - V4_LR_PR_AUC
print(f"  vs v3's logistic regression (PR-AUC {V3_LR_PR_AUC:.4f}, the pre-velocity baseline): {vs_v3:+.4f}")
print(f"  vs v4's logistic regression (PR-AUC {V4_LR_PR_AUC:.4f}, the 64-feature superset this strips down): {vs_v4:+.4f}")
if vs_v3 > 0.005:
    print("  Real PA-grain gain over v3 — the leaner fastball-velocity-direction feature set")
    print("  lets the signal show up without 22 other columns diluting it.")
elif vs_v3 < -0.005:
    print("  Worse than v3 — check for a wiring bug.")
else:
    print("  Still flat vs v3 — the #3-ranked feature's signal isn't enough on its own to move")
    print("  the aggregate, even isolated from the columns that diluted it in v4.")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1-v4)")
print("=" * 72)
...
# v3/v4's own game-grain results, hardcoded for reference.
V3_GAME_RELIABILITY = 0.0002
V3_GAME_RESOLUTION = 0.0133
V4_GAME_RELIABILITY = 0.0002
V4_GAME_RESOLUTION = 0.0136
...
print(f"  (v4's game-grain result, for reference: reliability {V4_GAME_RELIABILITY:.4f}, "
      f"resolution {V4_GAME_RESOLUTION:.4f}, verdict real_improvement)")
print("=" * 72)
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v5_fastball_velocity_direction",
            "target": TARGET,
