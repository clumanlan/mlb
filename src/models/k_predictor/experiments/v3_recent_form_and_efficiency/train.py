"""
k_predictor experiment v3: recent-form, opposing-lineup volatility, and pitch
efficiency/whiff-rate features.
Run from src/models/k_predictor/ with: python experiments/v3_recent_form_and_efficiency/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v3 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Extends v2 (batter/team K-rate, weather, TTO on top of v1's pitcher-workload
# set; PR-AUC 0.2816, game-grain real_improvement — the strongest model in this
# repo). Motivated by an ad-hoc residual dive (not a versioned experiment) on
# v2's total-K count-distribution check: the 20 worst 2024 total-K misses were
# almost all under-predictions (19/20), and three separate threads in that
# analysis each pointed at a different gap in v2's feature set. All three
# additions below turned out to need little-to-no new production code — the
# underlying machinery already existed, just never selected into any model's
# FEATURE_COLS.
#
#   1. TRAILING-3-START PITCHER FORM (window=3 instead of 'season'). v2's
#      rolling pitcher features all use window='season' — a cumulative
#      average that (a) resets to null on a pitcher's season opener, discarding
#      a hot finish to the prior season (confirmed on Bobby Miller's 2024-03-29
#      start: his elevated final 3 2023 starts were invisible to the
#      season-window feature) and (b) smooths away a 2-3-start hot streak into
#      a season-long average (confirmed rising trailing-3-game K/IP into 7 of
#      19 worst misses, e.g. Tanner Bibee 1.00->1.11->1.40->2.06 K/IP).
#      rolling_stats.build_pitcher_rolling_stats_all_roles and
#      build_pbp_pitcher_rolling_feats_all_roles already support int windows,
#      already tested for cross-season-boundary carry (see
#      test_rolling_sum_short_window_carries_across_season_boundary and the
#      window=10 tests on both _all_roles builders) — this is a second call
#      with window=3, zero new production code, same pattern
#      batters_faced_predictor v2/v7 already used for its own trailing-3-start
#      trend features.
#   2. OPPOSING-TEAM K-RATE VOLATILITY (season + trailing-5). v2 only has the
#      opposing lineup's K-rate LEVEL (opp_team_roll_season_pa_strikeout_rate).
#      10 of the 20 worst misses came against just 4 teams (Rockies x3, Red Sox
#      x3, A's x2, Cardinals x2) — all bottom-of-league 2024 offenses, where
#      thin lineup depth can spike a team's K rate on a given night well above
#      its own season average even though the level feature IS picking up
#      real signal (mean opposing season K rate across the 20 misses was
#      23.7%, above the ~22% league baseline — just not enough). New function
#      this session: rolling_stats.build_team_strikeout_volatility, mirroring
#      game_context.build_team_scoring_volatility's mean/std/max-of-per-game-
#      value shape (same _rolling_pooled_std/_rolling_max primitives) but on a
#      team's own per-game K rate instead of runs scored (TDD'd,
#      tests/hit_predictor/test_rolling_stats.py). Pulled at both season (long)
#      and trailing-5-game (short) windows per explicit direction — an
#      analogous scoring-volatility feature came back flat in
#      batters_faced_predictor v6, but that was a different target (batters
#      faced/game script) and mechanism (bullpen usage, not contact quality on
#      a given day), so this is tested independently rather than assumed to
#      fail.
#   3. PITCH EFFICIENCY + WHIFF RATE (season + trailing-3). User's own
#      intuition: an efficient pitcher (fewer pitches per batter) gets through
#      more of the lineup, which is a separate lever from raw "stuff" quality.
#      Both already exist as named RATE columns in
#      build_pbp_pitcher_rolling_feats's own final_cols — nobody has ever
#      selected them into a model's FEATURE_COLS:
#        - pa_pitch_count_mean: pitches per PA — the direct efficiency
#          measure, already point-in-time-safe (rolled numerator/denominator,
#          divided once).
#        - command_swinging_strike_rate: swinging-strike pitches / total
#          pitches — a true whiff rate from pitch-level Statcast data
#          (pitch_call already tagged is_swinging_strike in
#          hit_predictor.processing.pipeline.build_pbp_features, which
#          k_predictor's own pbp already carries). No model in this repo's
#          current architecture has wired pitch-level "stuff"/"command"
#          features into a model before now (hit_predictor's own
#          v1_pitcher_features/v5_tier1_features use the SEASON-level version
#          of these same columns, for a different target).
#      Both pulled from the SAME build_pbp_pitcher_rolling_feats_all_roles
#      calls #1 already needs (season and window=3) — no extra data pass.
#
# Evaluated the same way as v1/v2: PR-AUC primary vs best naive floor (PA
...
print(f"Feature count: {len(FEATURE_COLS)} (v2 had 22)")
...
# v1/v2's own results, hardcoded here for a direct comparison printout — not
# re-derived, since each used a smaller, frozen feature set.
...
print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v3 (trailing-3 form, opp-team volatility, whiff/efficiency)")
...
vs_v2 = best["pr_auc"] - V2_LR_PR_AUC
if vs_v2 > 0.005:
    print(f"  vs v2's logistic regression (PR-AUC {V2_LR_PR_AUC:.4f}): {vs_v2:+.4f} —")
    print("  recent-form/volatility/efficiency features add real PA-grain signal beyond v2.")
elif vs_v2 < -0.005:
    print(f"  vs v2's logistic regression (PR-AUC {V2_LR_PR_AUC:.4f}): {vs_v2:+.4f} —")
    print("  worse than v2 — check for a wiring bug before concluding the new features don't help.")
else:
    print(f"  vs v2's logistic regression (PR-AUC {V2_LR_PR_AUC:.4f}): {vs_v2:+.4f} — flat,")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1/v2)")
print("=" * 72)
...
# v2's own game-grain result, hardcoded for reference — reliability 0.0002,
# resolution 0.0131, verdict real_improvement.
V2_GAME_RELIABILITY = 0.0002
V2_GAME_RESOLUTION = 0.0131
print(f"\n  (v2's game-grain result, for reference: reliability {V2_GAME_RELIABILITY:.4f}, "
      f"resolution {V2_GAME_RESOLUTION:.4f}, verdict real_improvement)")
print("=" * 72)
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v3_recent_form_and_efficiency",
            "target": TARGET,
