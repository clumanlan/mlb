"""
k_predictor experiment v15: adds two in-game workload features -- uncapped PA
position and a projected pitch count -- to v14's feature set.
Run from src/models/k_predictor/ with: python experiments/v15_pa_workload/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                   v15 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# v14 added platoon_matchup on top of v6; verified via slice_verification.py
# to be a real, bootstrap-significant fix on the same-hand slice specifically,
# even though it was invisible in aggregate PR-AUC/game-grain metrics (see
# v14_results.md). v15 keeps platoon_matchup and adds two more features from
# the same "already have the ingredients, never combined them" bucket:
#
#   1. estimated_team_pa_position (NEW to FEATURE_COLS, zero new pipeline
#      code): the uncapped, continuous version of the PA-position signal
#      FEATURE_COLS already has capped at 3 as expected_times_through_order.
#      Capping at 3 throws away granularity the fatigue hypothesis needs --
#      a batter's 4th PA against a starter still cruising isn't the same as
#      his 4th PA against one who's clearly gassed, but both get the same
#      capped value of 3.
#   2. pitcher_projected_pitches_before_pa (NEW function, TDD'd in
#      pitcher_workload.py): (estimated_team_pa_position - 1) *
#      pitcher_roll_last3g_pa_pitch_count_mean -- projects actual pitch
#      count, not just batters faced, since two batters at the same PA slot
#      can represent very different real pitcher workload depending on pace
#      (deep counts, foul balls, working around traffic).
#
# Both are pre-game-knowable the same way expected_times_through_order
# already is (see expected_role.py and pitcher_workload.py's
# build_pitcher_projected_workload docstring for the reasoning). Everything
# else (data, tuning grids, eval) is identical to v14_platoon_matchup/train.py
# so the delta is attributable to these two features.
# ---------------------------------------------------------------------------- #
import yaml
...
# ------------------------- 2f. IN-GAME WORKLOAD, NEW in v15 ------------------ #
# estimated_team_pa_position: uncapped PA-position, already computed by
# create_pa_outcome_strikeout -- just never added to FEATURE_COLS before
# (only its capped-at-3 derivative, expected_times_through_order, was used).
# pitcher_projected_pitches_before_pa: TDD'd in pitcher_workload.py, multiplies
# that position by the trailing-3-game pitch-per-PA pace for a sharper
# fatigue proxy than PA-position or TTO alone.
#
# Pace must come from the STARTER'S OWN trailing-3g rolling stat, not the
# pitcher_roll_last3g_pa_pitch_count_mean merged in above -- that one is
# keyed on expected_pitcher_key_id/expected_pitcher_role, a pre-game estimate
# that deliberately falls back to team-pooled bullpen stats once a PA is
# projected to be past the starter's own expected depth (correct for season/
# rolling rate features, where at serving time you genuinely don't know which
# reliever it'll be). But create_pa_outcome_strikeout is already scoped to
# REALIZED pitcher_role=='sp' rows only -- every row here really is the
# starter -- so reusing that estimate-gated join for a feature meant to
# describe THIS pitcher's own workload silently swaps in his bullpen's pace
# once his outing runs long, exactly when a workload/fatigue signal should be
# sharpest. Join on starting_pitcher_id (the realized identity) instead.
...
FEATURE_COLS = [
    ...
    "estimated_team_pa_position",  # NEW in v15: uncapped in-game PA position
    "pitcher_projected_pitches_before_pa",  # NEW in v15: projected pitch count heading into this PA
    # 1. season (last season)
...
print(f"Feature count: {len(FEATURE_COLS)} (v14's 43 + estimated_team_pa_position + pitcher_projected_pitches_before_pa)")
