"""
k_predictor experiment v6: hyperparameter tuning on v3's feature set, the
production-candidate pass.
Run from src/models/k_predictor/ with: python experiments/v6_tuned/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v6 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# v3, v4, and v5 all landed on statistically indistinguishable PA-grain PR-AUC
# (0.2820 / 0.2836 / 0.2832) and game-grain resolution (0.0133 / 0.0136 /
# 0.0135) — the velocity/spin-decline feature hunt is closed for now. v3 is
# the leanest of the three (42 features) for the same performance, so it's
# the feature set carried forward here. Every prior version used
# LogisticRegression(max_iter=1000) and XGBClassifier(n_estimators=100)
# completely untuned defaults — this pass properly tunes both before calling
# any model a production candidate, per this project's own
# batters_faced_predictor history: default XGBoost badly overfit there
# (~15-20k rows), and a proper tune (shallow trees, low learning rate, early
# stopping) swung it from clearly losing to a hand-tuned formula to clearly
# beating it. LR has won every classifier built in this project so far, but
# never against a fairly tuned XGBoost — this is the first time that's
# actually been checked.
#
#   1. LOGISTIC REGRESSION: grid search over C x penalty (l1/l2) x
#      class_weight (None/balanced), scored on the untouched VAL_SEASON.
#   2. XGBOOST: grid search over max_depth x learning_rate (shallow trees,
#      low LR, following the batters_faced_predictor recipe), early-stopped
#      against the latest FIT_SEASON held out of training (same
#      CORE_FIT_SEASONS pattern batters_faced_predictor used), final model
#      selection still scored on the untouched VAL_SEASON.
#
# Evaluated the same way as v1-v5: PR-AUC primary vs best naive floor (PA
# grain), then the game-grain aggregation check ("1+ strikeout this game") via
# run_pa_vs_game_grain_check + summarize_verdict — the tuned winner only
# becomes a real production candidate if it also clears the game-grain bar,
# not just PA-grain PR-AUC.
# ---------------------------------------------------------------------------- #
import yaml
...
print(f"Feature count: {len(FEATURE_COLS)} (same as v3's 42 -- this pass tunes hyperparameters, not features)")
