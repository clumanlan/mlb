"""
k_predictor experiment v9: player-vs-league interaction features -- does the
model know a player's rate relative to the CURRENT league environment, not
just its raw level? -- on top of v8's league-wide-context feature set.
Run from src/models/k_predictor/ with: python experiments/v9_player_vs_league/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v9 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# v8 added league-wide rolling context but left it as raw levels only. v6's
# winning XGBoost config uses max_depth=2 -- a tree this shallow can only
# combine two features per tree, so an interaction between a weak-marginal
# feature (v8's league_roll_season_pa_strikeout_rate ranked #47 of 56) and a
# strong one (the player's own K rate) needs the greedy splitter to stumble
# onto exactly that pairing, which is unlikely when the weak feature rarely
# wins a split on its own. This pass precomputes PLAYER RATE / LEAGUE RATE
# explicitly -- the sabermetric "+"-stat pattern (wRC+/ERA+ are literally
# player-rate-over-league-rate, era-adjusted) -- to sidestep that shallow-tree
# discovery problem entirely, and does it comprehensively + reproducibly
# rather than as a one-off K-rate-only column.
#
# Two new production functions (interaction_feats.py, TDD'd this session --
# see implementation_plan.md), added alongside the file's existing
# find_rolling_trend_pairs/build_trend_features (same "auto-match column
# pairs by name" shape, different match rule):
#
#   1. find_player_vs_league_pairs -- pairs every non-league rolling column
#      with the league_roll_{window}_{stat} column sharing its EXACT window
#      bucket and stat name, regardless of the player-side entity prefix
#      (batter/pitcher/opp_team/pitching_team/anything). Unlike trend-pairing
#      (which deliberately spans a short window vs. season), a player's
#      window must match the league's window exactly here.
#   2. build_player_vs_league_features -- for every pair, adds
#      {entity}_vs_league_ratio_{window}_{stat} (player/league, zero-guarded)
#      and {entity}_vs_league_direction_{window}_{stat} (sign of the
#      difference) -- the same ratio+direction shape build_trend_features
#      already uses, not a plain diff (this file's own build_trend_features
#      docstring documents why a plain difference measurably hurt held-out
#      metrics in an earlier version: it's a linear combination of two
#      already-present raw columns a tree can reconstruct with one extra
#      split, while ratio/direction are genuine nonlinear/coarsened
#      transforms).
#
# "Comprehensive" here means column-name pattern matching over the FULLY
# ASSEMBLED pa_outcome frame, not a hand-picked subset -- it automatically
# finds every category with both a player-side and a league-side column
# present. Two merges were widened/added to expose categories that were
# already computed but not previously selected into pa_outcome: (a) batter/
# pitcher PA-outcome rolling now selects walk/hbp/single/xbh/hr rate too, not
# just strikeout rate (zero new feature-engineering -- both
# batter_pbp_rolling/pitcher_pbp_rolling already compute all six categories);
# (b) a new merge for the batter's own slash-line stats
# (batter_box_rolling_obp_slg, already computed for v7's best-batter
# extremum feature, never merged directly onto pa_outcome for the batter's
# own value before). This also means opp_team_roll_season_pa_strikeout_rate
# and pitching_team_roll_season_pa_strikeout_rate (both already in
# pa_outcome since v2/v7) auto-pair against the league column for free --
# confirmed by tracing the existing regex against those exact column names,
# not assumed.
#
# window='season' only this pass -- v8 never built a league_roll_last{N}g_
# table, so there's nothing for a short-window player column to pair
# against yet (a clean, separate future follow-up if this shows signal).
# Both raw (newly-wired) and derived (vs-league) columns are kept in
# FEATURE_COLS per the v3_interaction_feats/v4/v5 precedent, so feature
# importance can directly show whether the derived ratio adds anything
# beyond the raw level the model already has.
#
# Carries forward v6's exact winning hyperparameters (LR: C=0.1/L1/no class
# weighting; XGBoost: max_depth=2, learning_rate=0.03) rather than
# re-grid-searching, same convention as v7/v8. Evaluated the same way as
# v1-v8: PR-AUC primary vs best naive floor (PA grain), then the game-grain
# aggregation check, feature importance read off for the newly-wired raw
# columns and the derived vs-league columns SEPARATELY -- the whole
# comparison this experiment exists to make is whether the derived ratio/
# direction outranks the raw level it's built from.
# ---------------------------------------------------------------------------- #
import yaml
...
] + NEW_V9_RAW_COLS + NEW_V9_DERIVED_COLS
...
print(f"Feature count: {len(FEATURE_COLS)} (v8's 56 + {len(NEW_V9_RAW_COLS)} new v9 raw + {len(NEW_V9_DERIVED_COLS)} new v9 derived columns)")
print(f"Strikeout rate — train: {train_df[TARGET].mean():.3f}  val: {val_df[TARGET].mean():.3f}")
print("New v9 raw columns (newly wired, not newly computed):")
for c in NEW_V9_RAW_COLS:
    print(f"  {c}: {model_df[c].notna().mean():.1%} non-null, mean {model_df[c].mean():.4f}")
print(f"New v9 derived vs-league columns ({len(NEW_V9_DERIVED_COLS)} total, from {len(vs_league_pairs)} auto-discovered pairs):")
for c in NEW_V9_DERIVED_COLS:
    print(f"  {c}: {model_df[c].notna().mean():.1%} non-null, mean {model_df[c].mean():.4f}")
...
# v6's tuned XGBoost remains the standing best model after v7's and v8's flat
# results (v7 PR-AUC 0.2836, v8 PR-AUC 0.2836, vs v6's 0.2838) — hardcoded
# here for a direct comparison printout, same convention as every prior
# version's script.
...
print(f"\n{'='*72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v9 (player-vs-league interaction features)")
...
if vs_standing > 0.005:
    print(f"  vs v6's tuned XGBoost (PR-AUC {STANDING_BEST_XGB_PR_AUC:.4f}): {vs_standing:+.4f} — real gain from the player-vs-league interaction features.")
...
print("\n" + "=" * 72)
print("GAME-GRAIN CHECK — batter-game \"1+ strikeout\" (same approach as v1-v8)")
print("=" * 72)
...
colors = [
    "darkorange" if name in NEW_V9_RAW_COLS
...
ax.set_title("Feature importance — XGBoost (v9 raw new in orange, derived vs-league in red)")
...
print("\nv9 new RAW-column ranks (of {} features):".format(len(importances)))
for c in NEW_V9_RAW_COLS:
    rank = list(rank_order.index).index(c) + 1
    print(f"  {c}: rank #{rank}, importance {rank_order[c]:.4f}")
print("\nv9 new DERIVED vs-league column ranks (of {} features):".format(len(importances)))
for c in NEW_V9_DERIVED_COLS:
    rank = list(rank_order.index).index(c) + 1
    print(f"  {c}: rank #{rank}, importance {rank_order[c]:.4f}")
