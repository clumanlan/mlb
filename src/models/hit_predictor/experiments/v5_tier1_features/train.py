...
from models.hit_predictor.utils.calibration import run_isotonic_calibration_check
...
# Calibration check (ROADMAP.md backlog item 2 — BENCHMARKS.md §4.5's
# overconfidence finding). Isotonic is fit on half of game_results and
# reported on the OTHER half only, so the reported improvement isn't just
# the calibrator overfitting its own training data — see
# utils/calibration.py::run_isotonic_calibration_check. Smaller min_n than
# GAME_MIN_N since the calib-eval half is itself roughly half of game_results.
CALIB_FIT_FRAC = 0.5
...
for name, model in fitted_models.items():
    ...
    # Real MLflow run_id, obtained before any output path is built below, so
    # every file this model's iteration writes (plots, feature-importance
    # JSON, prediction parquet) lands under one namespaced directory instead
    # of overwriting the previous run's files in place. log_evaluation_to_
    # mlflow resumes this exact run further down via run_id=run_id.
    ...
    # Persist predictions alongside metrics: game_results is already the
    # game-grain predictions frame this call returns; pa_predictions is
    # built here (not inside run_pa_vs_game_grain_check) because it needs
    # play_id as the PA-grain key, which that function's own internal
    # pa_results doesn't carry (it's keyed on group_cols only).
    pa_predictions = val_df[['gamepk', 'play_id', 'batter_id']].copy()
    pa_predictions['is_hit'] = y_val.to_numpy()
    pa_predictions['pred_prob'] = proba
    pa_pred_path, game_pred_path = save_predictions(pa_predictions, game_results, RUN_DIR)
    ...
    # Isotonic recalibration check on the aggregated game-grain probabilities
    # (ROADMAP.md backlog item 2) — calibration only, doesn't touch the
    # separate discrimination-gap question game_metrics above already covers.
    calib_check = run_isotonic_calibration_check(
        game_results, fit_frac=CALIB_FIT_FRAC, random_state=42,
        n_bins=N_BINS, min_n=CALIB_MIN_N,
    )
    print(f"\n{'#' * 75}\n# GAME-GRAIN CALIBRATION: RAW vs ISOTONIC-CALIBRATED (calib-eval half)\n{'#' * 75}")
    for key in ("ece", "reliability", "resolution"):
        print(f"  {key:12s}  RAW={calib_check['raw'][key]:.4f}   CALIBRATED={calib_check['calibrated'][key]:.4f}")
    metrics.update({
        f"calib_eval_{split}_{k}": v
        for split, sub in calib_check.items()
        for k, v in sub.items()
    })

    calibration_plot_path = RUN_DIR / f"{MODEL_TYPE_TAGS[name]}_calibration_curve.png"
    plot_calibration_curve(
        y_val, {name: {"proba": proba}}, n_bins=N_BINS, min_n=MIN_N,
        save_path=calibration_plot_path,
    )

    artifact_paths = [str(calibration_plot_path), str(pa_pred_path), str(game_pred_path)]
    ...
    if name == "Random Forest":
        ...
        importance_plot_path = RUN_DIR / "random_forest_feature_importance.png"
        plot_feature_importance(importances, save_path=importance_plot_path)
        artifact_paths.append(str(importance_plot_path))

        importance_json_path = RUN_DIR / "random_forest_feature_importance.json"
        save_feature_importance_json(importances, save_path=importance_json_path)
        artifact_paths.append(str(importance_json_path))
    ...
    log_evaluation_to_mlflow(
        ...
        params={
            **model.get_params(),
            ...
            "SHORT_WINDOW_GAMES": SHORT_WINDOW_GAMES,
            "GAME_MIN_N": GAME_MIN_N,
            "CALIB_FIT_FRAC": CALIB_FIT_FRAC,
