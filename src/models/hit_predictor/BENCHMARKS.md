...
### Isolating which input drives error in a compound prediction (N × p targets)

Any total-count prediction built as "sum probability p over N estimated events" (k_predictor's total-K = sum of per-slot K-probability over an estimated batters-faced count; the same shape applies to bb_predictor's total-BB and any future total-hits model) has two separate error sources: the count estimate N being wrong, and the per-event probability p being wrong. It's tempting to reason about which one "matters more" from extreme thought experiments (N off by 10x caps the total regardless of p; p=0 zeroes the total regardless of N) — but that only shows either factor *can* dominate if catastrophically wrong, not which one *is* dominating today's actual, much smaller errors. Settle that empirically instead:

**Method.** Compute a post-hoc, hindsight-only diagnostic variable measuring how wrong the N estimate was for each row (e.g. `bf_gap = |realized_N - estimated_N|` — not usable as a pre-game feature, purely for slicing error). Sort into quantile buckets (quartiles are enough) by that variable, and look at the compound prediction's own error (MAE of predicted total vs. realized total) within each bucket.

**Reading it.** The bucket where N was nearly correct (closest quartile) isolates "how wrong is the compound prediction when the N-error source is ~silenced." If that bucket's MAE is still close to the *overall* MAE, N-error is not the dominant driver — most of the error persists even when N is right, so it must be coming from p (the per-event model's own discrimination) or genuine per-row noise no feature will ever explain. Only the *gap* between the closest-N-error bucket and the furthest-N-error bucket is attributable to N specifically. Worked example: k_predictor's total-K count-distribution check (2026-08-25/26, see `ROADMAP.md`'s k_predictor Mid-term entries) — closest-quartile MAE 1.724 vs. overall MAE 1.832 (94% of total error survives with N nearly correct), furthest-quartile MAE 2.078 (the N-attributable delta is only ~0.354, the minority contributor). This directly overturned an intuition that a bad N estimate must be the main lever, and correctly pointed at improving the per-event classifier (or building N as its own predictive model instead of a shrinkage average) as the higher-value next move.

**Caveat.** This is correlational bucketing, not a controlled experiment or a formal variance decomposition — the quantile buckets could differ on other axes besides N-accuracy (e.g. starts with an early pull might be both harder to estimate N for and just generally more chaotic games, inflating both bf_gap and MAE for reasons unrelated to N itself). Treat the closest-bucket "floor" reading as solid (it only requires that bucket's N-error to genuinely be small, which quantile sorting guarantees by construction) and the exact bucket-to-bucket delta as directional, not a precise causal split.
...
- [x] Devig helper — **done 2026-08-31** via `utils/odds_math.py`'s `american_to_implied_prob`/`devig_two_way` (proportional method), built for k_predictor's first real-odds backtest (see `ROADMAP.md`'s 2026-08-31 entry). Model-agnostic — any model with a two-sided DK line can use it, not just k_predictor.
- [x] Edge-vs-vig report — **done 2026-08-31**, scoped to k_predictor's `pitcher_strikeouts` prop rather than hit_predictor's own PA/game grain: `src/models/k_predictor/backtest/edge_report.py`, using `compute_edge()` (also in `odds_math.py`) and `count_distribution.py`'s new `prob_exceeds_line`. Result was inconclusive/leaning negative on a small (131-start) sample — see `ROADMAP.md` for the full number. The hit_predictor-specific version (per-PA/game, not per-pitcher-total) is still unbuilt if wanted later.
- [ ] CLV backtest scaffold — still open. k_predictor's 2026-08-31 backtest used a single closing-ish snapshot per game (commence_time), not a true CLV comparison (opening line vs. closing line vs. bet-placed line) — that would need multiple historical snapshots per game, not yet pulled for any model.
...
- **Heaton & Mitra, "Learning to Describe Player Form in the MLB"** (arXiv:2109.05280) — found 2026-08-31 via a "competitive with sportsbooks" search snippet, primary source opened and read directly 2026-09-01 (`arxiv.org/html/2109.05280v1`) after that snippet turned out to be unverifiable. **Correction: the "competitive with 3 sportsbooks, ~50% more coverage" claim does not appear anywhere in this paper and should not have been recorded — it was likely conflated from an oversold secondary description, not the primary source.** What the paper actually is: an **unsupervised representation-learning paper, not a prediction paper**. It learns 72-dimensional "form" embeddings from 2015-2018 play-by-play (750k PA, 2.9M pitches) via contrastive learning; evaluation is purely qualitative cluster-visualization for 4 sample players (Harper, Trout, Stanton, Walker, Cole) against stats-based clusters — **no supervised prediction task, no accuracy/AUC/log-loss number, and no sportsbook comparison exists anywhere in the document.** The authors' own framing: this is "a first step" and they "believe these embeddings could be utilized to predict... in-game and game-level events" — explicitly future work, not something built or measured here. Real, legitimate paper and a fine citation for "learned player-form embeddings exist as a technique in MLB analytics," but **not a benchmark of any kind** — remove it as a comparison point, keep it only as a pointer to the embedding technique itself if that thread gets picked back up.
- A 2025 MDPI paper claiming accuracy 0.89–0.93 / AUC 0.97–0.98 for "baseball outcome prediction" — **resolved 2026-09-01, and it's worse than unverified.** The paper (MDPI *Applied Sciences* 15(13):7081) predicts **team win/loss**, not batter hits or pitcher strikeouts, on 859 games in the **Chinese Professional Baseball League — not MLB**, a smaller and far less efficiently-bet league than anything this project or the market backtest cares about. Its features include season-level cumulative sabermetrics (wRC+, wRAA, PLOB%) evaluated with plain 5-fold cross-validation rather than a time-based split — a textbook leakage setup, since those season-cumulative stats already encode information about the outcome being predicted. That combination is almost certainly the real source of the 0.97 AUC. **Wrong league, wrong target, real methodology red flag — not a comparable benchmark for anything in this repo, do not use even directionally.**
...
§4.1's Heaton & Mitra paper was initially recorded here as a K-prediction precedent — **corrected 2026-09-01 after reading the primary source directly**: it does not predict strikeouts, hits, or anything supervised at all — it's an unsupervised player-form-embedding paper with no prediction task, no metrics, and no sportsbook comparison. Removed as a benchmark; still no citable academic accuracy/calibration number exists for this target.
...
6. **Don't reuse the unreliable numbers** (MDPI 2025 paper — resolved as wrong-league/wrong-target/leakage-risk, not just unverified — and FanGraphs Outcome Machine's 96%) as targets or comparisons — both are flagged unreliable above.
...
- Heaton & Mitra, "Learning to Describe Player Form in the MLB" — arXiv:2109.05280 (embedding-technique reference only, not a benchmark — see §4.1 correction)
...
- MDPI (2025) *Applied Sciences* 15(13):7081 baseball outcome prediction paper — found and resolved: Chinese Professional Baseball League team win/loss, not MLB batter/pitcher props; leakage-risk methodology; not used as benchmark
