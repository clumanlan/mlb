...
- **partial** — computable with a caveat (missing a component column, needs approximation, or needs external context)
...
|---|---|---|---|
| **Exit Velocity (EV)** | Speed of the ball off the bat (mph) at contact | Single strongest predictor of batted-ball outcome quality — hard-hit balls become hits/XBH far more often regardless of luck | have it — `launch_speed` |
| **Launch Angle (LA)** | Vertical angle the ball leaves the bat (0° = parallel to ground, negative = grounder) | Combined with EV, determines grounder/liner/fly-ball/pop-up. 105mph at −20° is a routine grounder — EV alone is an incomplete story | have it — `launch_angle` |
| **Spin Rate** | RPM of the pitch as it leaves the hand | Higher spin four-seamers = more perceived "rise" (less drop), inducing whiffs up in the zone; higher spin breaking balls = sharper/later break | have it — `spin_rate` |
| **Spin Axis / Direction** | Tilt/orientation of the spin (clock-face notation) | Determines *which direction* the ball moves — two pitches with identical spin rate can move completely differently by axis | have it — `spin_direction` |
| **Pitch Velocity** | Speed of the pitch (mph), out of hand and at plate | Baseline stuff quality; also the input for velocity-differential-between-pitch-types deception features | have it — `start_speed`, `end_speed` |
| **Extension** | Distance (ft) from the rubber to release point | Releasing closer to the plate gives hitters less reaction time — makes a given velocity "play up" | have it — `extension` |
| **Sprint Speed** | Max run speed (ft/sec) on a competitive run | Proxy for raw speed independent of stolen-base totals/instincts — feeds defense/baserunning value models | gap — not in `pbp` or box score; would need a separate Statcast sprint-speed pull |
| **Route Efficiency / OAA inputs** | Distance covered vs. optimal path, reaction time, burst | Raw inputs to Outs Above Average (below) | gap — not tracked in current ingestion |
| **Release Point (x, y, z)** | 3D coordinates of ball release | Used to detect pitch tunneling (multiple pitch types released from the same point/trajectory to deceive hitters) | have it — `release_pos_x/y/z` |
| **Plate Location (px, pz)** | Where the pitch crosses the front of the plate | Basis for zone-based features (edge%, chase rate, called-strike probability) | have it — `plate_x`, `plate_z`, `zone` |
...
- **In this repo:** partial — have `launch_speed` + `launch_angle`, so an approximation is computable now; the real MLB definition uses a full speed/angle curve, not a single threshold.
...
- **Chase Rate (O-Swing%)** = swings at pitches outside the zone / **pitches outside the zone**. Partial — implemented with a known denominator gap — `_create_batter_plate_discipline_stats` → `batter_season_o_swing_rate` computes `is_chase.mean()` over **all pitches**, not pitches-outside-zone only. Inherited unchanged from the pre-existing pitcher version (`_create_pitcher_stuff_command_stats` → `command_chase_rate`), which has the same gap. Two batters with identical true chase discipline can show different `o_swing_rate` values purely because one saw more out-of-zone pitches than the other. Flagged, not yet fixed — deferred pending a decision on whether to also change the already-shipped pitcher column.
...
- **What it measures:** `command_called_strike_rate` + `command_swinging_strike_rate` — the share of all pitches that result in either a called strike or a swing-and-miss. Widely cited in sabermetrics as the single best predictor of K% (R² ≈ 0.59), stronger than either component alone, because it captures both zone command and swing-and-miss stuff in one number.
- **In this repo:** **have it** — `command_csw_rate`, pitcher-side only, at both season/last-season grain (`season_stats.py`'s `_create_pitcher_stuff_command_stats`) and rolling grain (`rolling_stats.py`'s `build_pbp_pitcher_rolling_feats`), a plain sum of the two already-existing rate columns. Added for `k_predictor` v11 after a residual-correlation screen found it the strongest of 8 candidate features tested against v6's held-out residuals (called-strike rate alone was weak, r=0.0341; the composite was r=-0.1705, ~4x stronger). **Result: flat vs. v6 in aggregate** despite the strong screen correlation — see `ROADMAP.md`'s v11 entry for the full methodology finding (a real, non-spurious correlation doesn't guarantee enough magnitude to clear this project's aggregate-improvement bar). Ranks respectably (#22/#23 of 116 features) — real, non-trivial model usage, not dead weight.
...
- **In this repo:** Partial — `BATTER_BOXSCORE_COLUMNS` has `singles`, `doubles`, `triples`, `hr`, `bb`, `ab` — but no `hbp`, `ibb`, or `sf` columns. Computable with fixed published weights as a v1 approximation; a season-specific-weight version needs those additional counting fields.
...
- **In this repo:** FIP is already implemented in `_create_pitcher_pa_outcome_stats` (`fip_hr`, `fip_bb`, `fip_k`, constant `C = 3.10`). xFIP/SIERA are a gap — need league-average HR/FB rate (xFIP) and batted-ball-type regression coefficients (SIERA), neither currently computed.
...
- **In this repo:** partial — implemented as an approximation — `_create_boxscore_batter_stats` → `batter_season_babip` (zero-denominator-guarded). Missing `sf` (sac flies) from `BATTER_BOXSCORE_COLUMNS`, so the SF adjustment in the formula above is omitted.
...
- **In this repo:** partial — implemented as a v1 approximation — `processing/features/park_factors.py` → `build_park_factors(schedule, batter_boxscore)`. `venue_id` comes from `schedule` (`SCHEDULE_COLUMNS`), joined to `batter_boxscore` via `gamepk` to compute each venue's hit rate (h/ab) for a season relative to the league-wide hit rate that season — `> 1` = hitter-friendly, `< 1` = pitcher-friendly. This is a single-season index (trailing 1 year, `_shift_to_last_season`-shifted like every other season feature here), **not** the standard 3-year rolling window described above, and it's a single hit-rate factor rather than separate runs/HR/2B components or the home-vs-road-split methodology — both are real simplifications, deferred as future work. Still a prerequisite for wRC+ once that's built. A strikeout-specific sibling, `build_park_strikeout_factor(schedule, pitcher_boxscore)`, was added for k_predictor's v10 pass (**partial**, same shape/limitations as the hit-rate version above — season-level not 3-year, k/ip instead of h/ab, matching `season_stats.py`'s own `k_rate` convention) — flat in aggregate model metrics, mid-pack (#40 of 114) in feature importance; see `k_predictor/experiments/v10_count_leverage_park_k/v10_results.md`.
...
|---|---|---|
| **Weather (temp, wind speed/direction)** | Air density affects carry distance; wind direction relative to outfield orientation adds/subtracts HR distance; warmer air carries further | have it — `weather_temp`, `weather_wind_speed`, `weather_wind_direction` already parsed in `process_game_info` (`pipeline.py`) |
...
| **Days of rest / bullpen usage (fatigue proxies)** | Recent pitch counts, back-to-back appearances, rest days are leading indicators of stuff decline or injury risk | partial — `pbp` has per-game pitch counts (`_create_pitcher_pitch_count_stats` already computes these); day-of-rest itself needs a date-diff against the player's prior appearance, not yet computed |
...
| **Batted-ball spray angle / pull%** | Combined with EV/LA, indicates whether a power profile is exploitable by shifting/positioning | partial — have `hit_coord_x`/`hit_coord_y`; spray angle is computable from those, not yet implemented |
...
| Pitcher pbp-derived (stuff/command/PA-outcome+FIP/last-inning/pitch-count/contact-quality/two-strike-reach+put-away), role-aware | `build_pbp_pitcher_rolling_feats_all_roles(pbp, window)` | `build_pbp_pitcher_feats_all_roles` |
| Batter pbp-derived (PA-outcome/plate-discipline/in-play-contact/foul-contact/two-strike-foul/two-strike-reach+put-away) | `build_pbp_batter_rolling_feats(pbp, window)` | `build_pbp_batter_feats` |
| Team-level batter PA-outcome rates (K rate, walk/hit/HBP rate), starting lineup only | `build_team_batter_strikeout_rolling_feats(pbp, batter_boxscore, window)` / `build_team_batter_onbase_rolling_feats(pbp, batter_boxscore, window)` | none (team grain didn't exist at season level) |
| Team K-rate ceiling/volatility (mean/std/max of the team's own per-game K rate, not just its level) | `build_team_strikeout_volatility(pbp, batter_boxscore, window)` | `build_team_batter_strikeout_rolling_feats`'s own `pa_strikeout_rate` (level only) |
| Fastball-only pitcher velocity/spin + pitch-mix level/volatility, role-aware | `build_pitcher_fastball_rolling_feats_all_roles(pbp, window)` | `build_pbp_pitcher_rolling_feats_all_roles`'s own `stuff_start_speed_mean`/`stuff_spin_rate_mean` (pooled across every pitch type) |
| League-wide PA-outcome rates (K/BB/HBP/single/XBH/HR), unfiltered, keyed by `(game_season, game_date)` only | `build_league_pa_outcome_rolling_feats(pbp, window)` | `season_stats.build_league_pa_outcome_stats` — that one is a static last-season snapshot, not documented elsewhere in this file (see the written entry below) |
| League-wide batter slash-line stats (BA/OBP/SLG/ISO/BABIP), unfiltered, keyed by `(game_season, game_date)` only | `build_league_batter_rolling_stats(batter_boxscore, window)` | `build_batter_rolling_stats` — same formulas, pooled to the whole league |
...
**Team-level batter strikeout/on-base pooling** (`build_team_batter_strikeout_rolling_feats`/`build_team_batter_onbase_rolling_feats`, added for `k_predictor` v2 / `batters_faced_predictor`) — pools **starting-lineup batters only** (those carrying a `batting_order` that game, via `_create_batting_order`) into one row per `(team_id, gamepk)`, same "roll sums, divide once" rate rule as everything else in this file. Role-agnostic output: the same rolled table doubles as "the opposing lineup's rate" (merge on `batter_team_id`) or "the pitcher's own team's rate" (merge on `pitcher_team_id`) without needing a second function.

**Team K-rate volatility** (`build_team_strikeout_volatility`, added for `k_predictor` v3, 2026-08-27) — same lineup-pooling shape as the level function above, but mirrors `build_team_scoring_volatility`'s (§5F) mean/std/max-of-the-per-game-value treatment applied to a team's own K rate instead of runs scored: captures a team's strikeout-rate *ceiling*, not just its average — a thin-lineup team can spike well above its own season K rate on a given night even if that average looks unremarkable. **Tried, gap**: wired into `k_predictor` v3 (season + trailing-5-game windows) motivated by 10 of 20 worst 2024 total-K misses coming against just 4 bottom-of-league offenses — came back flat, all 6 columns ranking in the bottom third of feature importance, the same conclusion `build_team_scoring_volatility` already reached for a different model/target. Level, volatility, season and trailing windows have now all been tried for opposing-team K rate and none rank meaningfully.

**Fastball-only pitcher stuff + pitch mix** (`build_pitcher_fastball_rolling_feats`/`_all_roles`, `FASTBALL_PITCH_TYPES = {'Four-Seam Fastball', 'Sinker', 'Cutter'}`, added for `k_predictor` v4/v5, 2026-08-27) — `build_pbp_pitcher_rolling_feats`'s own `stuff_start_speed_mean`/`stuff_spin_rate_mean` (above) pool **every pitch type** a pitcher threw that game; since a fastball (~93-97mph) and a breaking/offspeed pitch (~78-88mph) start from very different baselines, a pitch-mix shift alone (more sliders, fewer fastballs) can move that pooled average with zero real change in arm strength. This isolates fastball-only start_speed/spin_rate, plus the pitch-mix level and volatility (`fastball_pitch_rate`/`_std`/`_max`, same mean/std/max-of-per-game-value shape as the team K-rate volatility above). `FASTBALL_PITCH_TYPES` was set via a same-session pitch-mix audit (2024, 623 qualified pitchers): 27.4% have a non-fastball primary pitch, Cutter the single largest such group (37 of 171) — included here since velocity-wise a cutter sits close to a four-seamer, not a breaking ball; the remaining ~21.5% whose primary pitch is a genuine breaking/offspeed pitch is a real, separate gap this fixed bucket doesn't close (would need a per-pitcher, season-level "actual primary pitch" assignment, not built). **Confirmed real, still a small effect**: at every matching window, the fastball-only velocity/spin feature outranks its all-pitch-pooled counterpart in `k_predictor` v4's feature importance (e.g. trailing-3 fastball velocity #6 of 64 vs. the pooled version ~#19) — the isolation hypothesis holds. `pitcher_trend_direction_fastball_start_speed_mean` (§5E's `build_trend_features`, direction only) ranked #3 of 64, the single strongest individual feature found in this project's history, but didn't move the aggregate PA-grain/game-grain metrics enough to clear this project's "real" threshold even after v5 isolated it from every other new column — a real, reusable lead, not yet a production feature. Pitch-mix rate/volatility itself did not rank meaningfully, same fate as team K-rate volatility above.

**League-wide rolling context** (`build_league_pa_outcome_rolling_feats`/`build_league_batter_rolling_stats`, added for `k_predictor` v8, 2026-08-29) — does the model know what season/era it's in (secular shifts like the 2023 pitch-clock/shift-ban/bigger-bases rules, or a juiced/dead-ball year)? Before this, the only league-wide table anywhere in this repo was `season_stats.build_league_pa_outcome_stats` — a STATIC last-season snapshot (K/BB/HBP/single/XBH/HR rate, keyed on `game_season` alone) used once, as the log5 denominator in `tier1_feats.build_log5_matchup_features` (a feature tried once in `hit_predictor` v5 and found marginal); `k_predictor` never imported it in any version. A last-season number lags a full year behind any real shift, so it can't answer "what does the league look like *right now*." These two functions are the rolling, in-season-updating equivalent, pooled across the **entire league** — every team, every batter, no starting-lineup filter, unlike every team-level feature elsewhere in this section — since league context isn't a matchup concept the way opposing-lineup features are. `build_league_pa_outcome_rolling_feats` covers the same six PA-outcome categories as the static table (rolling K/BB/HBP/single/XBH/HR rate); `build_league_batter_rolling_stats` adds the slash-line/sabermetric side (rolling BA/OBP/SLG/ISO/BABIP), the rolling equivalent of `build_batter_rolling_stats` pooled to the whole league. Both are keyed by **`(game_season, game_date)` only** — the first feature tables in this repo with no `gamepk`/`team_id`/`personId` key, since there's no single per-game "entity" to roll league-wide the way one team/pitcher/batter has (many games share a date): each pools to one row per calendar date first, rolls with a constant entity column (`shift(1)` at the date grain, so no same-day leakage), and merges onto a target table via a plain two-column merge — every PA on a given date gets an identical value by construction, since it isn't scoped to any one game. **Tried, flat-with-a-twist**: wired into `k_predictor` v8 (season window only) — PA-grain/game-grain metrics came back flat vs. v6/v7 (same "additive feature, no aggregate movement" pattern seen throughout this project), but feature importance was NOT uniformly weak: league walk rate and single rate ranked in the top quartile of 56 features, while league strikeout rate — the feature most directly on-target for the stated hypothesis, and the most redundant with K-rate signal the model already has from `k_predictor` v1-v3 — ranked #47 of 56, in the bottom fifth. See `v8_results.md` and `ROADMAP.md`'s 2026-08-29 entry for full numbers. Not yet tried: a trailing-N-game league window (only `window='season'` was tested), and a third league-wide pitcher-composite table (WHIP/FIP — would need a new innings-pitched aggregation path, not a 1:1 mirror of an existing rolled table like the two built here).

**Count-leverage / put-away rate** (`pa_two_strike_reach_rate`, `pa_put_away_rate`, added to `_pitcher_pa_outcome_per_game`/`_batter_pa_outcome_per_game` and their rolling wrappers for `k_predictor` v10, 2026-08-30) — every PA-outcome rate documented above describes the overall outcome or the *final* count reached (`pa_full_count_rate` etc.); none describe the leverage point of "did this PA ever get to 2 strikes, and what happened once it did." `pa_two_strike_reach_rate` = PAs where the MAX `count_strikes` across every pitch in the PA reaches 2 (not just the final pitch's count — a foul-off at 2 strikes keeps `count_strikes` at 2 on every following pitch, so checking only the final pitch would undercount a PA that reached 2 strikes early then, hypothetically, showed a lower final-pitch count) / total PAs. `pa_put_away_rate` = strikeouts / PAs-that-reached-2-strikes — a genuinely different, *conditional* number from `pa_strikeout_rate`, even though both derive from the same underlying counts (reach-rate × put-away-rate reconstructs the unconditional strikeout rate almost exactly, a real internal-consistency check, not just a plausibility eyeball). Built for both pitcher and batter sides. **Tried, split result**: wired into `k_predictor` v10 (season window only) — flat in aggregate PA-grain/game-grain metrics (same pattern as v7/v8/v9), but both two-strike-*reach*-rate columns placed in the top quartile of 114 features (pitcher #12, batter #16) while the pitcher's own *put-away* rate — the feature this thread was actually built to test — ranked #112 of 114, essentially unused (the batter-side put-away rate escaped this, still placing at #19). Likely mechanism for the pitcher-side collapse: put-away rate is arithmetically `pa_strikeout_rate / pa_two_strike_reach_rate`, and both inputs are already in the model's feature set, leaving little for a shallow (`max_depth=2`) tree to gain from the precomputed quotient — plausible, but doesn't explain the batter-side asymmetry, an open question. See `k_predictor/experiments/v10_count_leverage_park_k/v10_results.md` for full numbers. Not yet tried: a trailing-N-game window (only `window='season'` was tested).
...
**Player-vs-league interaction pair-finding (`find_player_vs_league_pairs`/`build_player_vs_league_features`, added for `k_predictor` v9, 2026-08-29)** — same "auto-match column pairs by name" shape as `find_rolling_trend_pairs`/`build_trend_features` above, but matching a player-level rolling column against the `league_roll_{window}_{stat}` column sharing its EXACT window bucket (not a short-window-vs-season comparison — a player's season rate must compare against the league's season rate, not some other window). `find_player_vs_league_pairs(columns)` works regardless of the player-side entity prefix (`batter`/`pitcher`/`opp_team`/`pitching_team`/anything), so it's comprehensive by construction — any model merging a new player-level and league-level rolling table together gets vs-league features for free, no hand enumeration. Excludes `_SAMPLE_SIZE_SUFFIXES` (`plate_appearances`/`pa_total`/`ab`/`ip`/`n_pitches`) from both sides of the pairing — a real bug caught mid-pass: a pitcher's own season PA COUNT auto-paired against the league's cumulative PA count purely because the stat names matched, producing a meaningless ratio (mean 0.0037, constant direction -1.0) that still ranked #31 of 111 in importance before the fix. `build_player_vs_league_features(df, pairs)` emits `{entity}_vs_league_ratio_{window}_{stat}` (player/league — the sabermetric "+"-stat pattern, wRC+/ERA+ are literally player-rate-over-league-rate) and `{entity}_vs_league_direction_{window}_{stat}` (sign of the difference), the same ratio+direction (not plain-diff) shape as `build_trend_features` above, for the same documented reason. **Tried, mechanism confirmed but no new information**: wired into `k_predictor` v9 (20 auto-discovered pairs, `window='season'` only — v8 never built a trailing-window league table) motivated by v6's shallow `max_depth=2` XGBoost potentially being unable to discover a weak-marginal-feature interaction via greedy splitting alone. `batter_vs_league_direction_season_pa_strikeout_rate` ranked #1 of 109 features (importance 0.3801, over a third of total gain, 5x the #2 feature) — confirming the model readily adopts a precomputed threshold — but PA-grain PR-AUC and game-grain resolution came back exactly equal to v6's own numbers, meaning the dominant feature is a more efficient *encoding* of information the raw batter K-rate already provided (league K-rate barely moves within a season, so the direction feature is close to a pre-thresholded binarization of the raw rate), not new predictive signal. Roughly two-thirds of the 38 derived columns showed exactly 0.0000 importance — signal concentrated almost entirely in the K-rate category (both batter and pitcher side), not the broader walk/hbp/single/xbh/hr/slash-line categories tried alongside it. See `v9_results.md` and `ROADMAP.md`'s 2026-08-29 entry for full numbers.

**Tested against a linear model for the first time in `k_predictor` v4/v5 (2026-08-27)** — every prior use of `build_trend_features` (this section, `batters_faced_predictor` v7) only ever fed a tree (Random Forest or XGBoost), which can reconstruct a plain difference from two raw levels via an extra split, explaining why diff-vs-ratio/direction mattered so much for trees specifically. A **plain difference is a linear combination LR could already represent internally too** (coefficients of +1/−1 on the two raw levels), so it was never expected to add anything there either — but **ratio and direction are genuine nonlinearities a linear model cannot construct from raw inputs on its own**, which is the reasoning `k_predictor` v4/v5 tested directly (fastball velocity/spin pairs only, `direction` kept and `ratio` dropped after v4 showed direction massively outranking ratio for the same stat, #3 vs. ~#24 of 64). Result: real per-feature importance (see §5D's fastball entry) but not enough to move the aggregate PA-grain/game-grain decision metrics past this project's "real" threshold — direction-vs-ratio-for-linear-models is a confirmed, reusable distinction now, independent of whether this specific application panned out.
...
**Team scoring volatility** — `build_team_scoring_volatility(schedule, window)`, added for `batters_faced_predictor`'s v6 (2026-08-26). Same `_team_game_long`/window convention as team win/loss record above, but rolls mean/std/max of per-game runs scored instead of win/loss level — captures a team's capacity for an explosive/blowout game, not just its average scoring. Reuses `rolling_stats.py`'s existing `_rolling_max` (for max) and `_rolling_pooled_std` (for std, off a locally-derived `team_score_sumsq` column — not persisted back into `_team_game_long`'s own output); mean falls out of the std call's own rolled sum/count as a free byproduct. **Tried, gap**: wired in as an opposing-team feature (`batters_faced_predictor` v6) and came back flat — none of mean/std/max ranked near the top of feature importance, `runs_scored_max` best-placed at #13 of 33. Note `_rolling_pooled_std` orders by `game_date`, not `game_datetime` — unlike win/loss record and rest days, it doesn't special-case same-day doubleheader ordering; a pre-existing limitation shared by every other `_rolling_pooled_std` caller in this file, not something this function fixes.
