"""
batters_faced_predictor experiment v4: pitcher_anomaly_count_this_season —
a season-to-date count of a pitcher's own strictly-prior anomalously-short
starts this season.
Run from src/models/batters_faced_predictor/ with:
  python experiments/v4_anomaly_count/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v4 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Motivated by experiments/residual_error_analysis_v2/ (val MAE 2.6471 for
# v2's tuned XGBoost): the dominant remaining failure mode is still established,
# high-confidence starters (expected_batters_faced_weight 0.7-0.86) pulled
# after 0-2 clean innings with no decision recorded — the same mode v2
# targeted (trend/rest/workload_density) but only partially closed
# (established-bucket bf_gap MAE moved 2.6053 -> 2.5154, not fully resolved).
#
# A follow-up volatility check (game_log_check.py, error_analysis_v2.md,
# game_log_check.md) found this is driven by 1-3 anomalous starts per
# pitcher-season (42-96% of that pitcher's own season variance concentrated
# in just those starts) across ALL 10 flagged pitchers checked — not chronic
# game-to-game unpredictability. That ruled out a trailing-3-5-start rolling
# std-dev/CV feature: a short window can't see a rare, non-clustering event
# coming. Rest days (pitcher_days_since_last_start: 6,6,5,6,7,6,8,5,6 across
# the 9 clearest events — normal-to-extra rest, never short), weather
# (mostly Clear/Cloudy, no rain), day-of-week, and opponent quality were all
# directly checked against these specific events and ruled out as shared
# pre-game-knowable triggers.
#
# v4 therefore tracks THAT a pitcher has already shown this pattern this
# season, not WHY:
#
#   PITCHER_ANOMALY_COUNT_THIS_SEASON: game_context.build_pitcher_anomaly_
#   count_this_season(start_df, anomaly_threshold=0.6, min_weight=0.3)
#   (new, TDD'd — tests/hit_predictor/test_game_context.py). A prior start
#   counts as anomalous if realized_batters_faced < 0.6 * that start's own
#   expected_batters_faced AND expected_batters_faced_weight >= 0.3 (the
#   weight gate keeps a cold-start pitcher's still-unreliable cascade
#   estimate from being spuriously flagged — a separate, already-closed
#   problem, see v3). Point-in-time safe: shift(1) before cumsum, so a
#   start's own anomaly never counts toward its own feature value; resets
#   each game_season.
#
# Every v1/v2 feature stays in the feature list unchanged (additive, not a
# replacement). Evaluated the same way as v2/v3: MAE primary vs v2's own
# tuned-XGBoost floor (2.6471), RMSE/Bias/Pearson r secondary, same
...
# tuned-XGBoost hyperparameters as v1/v2/v3 (carried over unchanged, per
# repo convention — see experiments/xgb_vs_cascade_diagnostic/run.py).
# ---------------------------------------------------------------------------- #
import yaml
...
# ------------------------- 8. ANOMALY COUNT (NEW, v4) ------------------------ #
anomaly_count = game_context.build_pitcher_anomaly_count_this_season(
    start_outcome[[
        "personId", "gamepk", "game_season", "game_date",
        TARGET, "expected_batters_faced", "expected_batters_faced_weight",
    ]].rename(columns={TARGET: "realized_batters_faced"}),
    anomaly_threshold=0.6, min_weight=0.3,
)
start_outcome = start_outcome.merge(anomaly_count, on=["personId", "gamepk"], how="left")
...
FEATURE_COLS = [
    ...
    # v4 feature (this pass)
    "pitcher_anomaly_count_this_season",
]
...
print(f"pitcher_anomaly_count_this_season — val distribution:\n{val_df['pitcher_anomaly_count_this_season'].value_counts().sort_index()}")
...
_eval("Linear regression (v4)", y_val.to_numpy(), lr.predict(Xval_sc))

print("Training XGBoost (v4, default)...")
import xgboost as xgb
xgb_model = xgb.XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
xgb_model.fit(Xtr, y_train)
_eval("XGBoost (v4, default)", y_val.to_numpy(), xgb_model.predict(Xval))

print("Training XGBoost (v4, tuned)...")
# Hyperparameters and early-stopping setup carried over unchanged from v1/v2/v3.
...
_eval("XGBoost (v4, tuned)", y_val.to_numpy(), xgb_tuned.predict(Xval_core))
...
print(f"\n{'=' * 72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v4 (pitcher_anomaly_count_this_season)")
...
if vs_v2 < -0.02:
    print(f"  vs v2's tuned XGBoost (MAE {V2_TUNED_MAE:.4f}): {vs_v2:+.4f} — the anomaly-count")
    print("  feature adds real signal beyond v2's feature set.")
elif vs_v2 > 0.02:
...
# ── 6d. Established-starter bucket re-check (the specific failure mode this v4 targets) ──
...
# ── 6e. Anomaly-count-specific slice: has this pitcher already had an anomaly? ──
print(f"\n{'=' * 72}\nANOMALY-COUNT SLICE — has_prior_anomaly_this_season vs. never\n{'=' * 72}")
has_anomaly = val_df["pitcher_anomaly_count_this_season"] >= 1
mae_has_anomaly_cascade = check_df.loc[has_anomaly, "cascade_bf_gap"].mean()
mae_has_anomaly_new = check_df.loc[has_anomaly, "new_bf_gap"].mean()
mae_no_anomaly_cascade = check_df.loc[~has_anomaly, "cascade_bf_gap"].mean()
mae_no_anomaly_new = check_df.loc[~has_anomaly, "new_bf_gap"].mean()
print(f"has prior anomaly (n={has_anomaly.sum()}):     cascade MAE {mae_has_anomaly_cascade:.4f}  {best_model_name} MAE {mae_has_anomaly_new:.4f}")
print(f"no prior anomaly  (n={(~has_anomaly).sum()}):     cascade MAE {mae_no_anomaly_cascade:.4f}  {best_model_name} MAE {mae_no_anomaly_new:.4f}")
print("=" * 72)
...
ax.set_title("Feature importance — XGBoost (v4, tuned)")
...
# ── 9. Write v4_results.md ────────────────────────────────────────────────────
md_lines = [
    f"# v4 Results — {MODEL_NAME} (pitcher_anomaly_count_this_season)",
    "",
...
md_lines += [
    ...
    "",
    f"has prior anomaly (n={has_anomaly.sum()}): cascade MAE {mae_has_anomaly_cascade:.4f} | "
    f"{best_model_name} MAE {mae_has_anomaly_new:.4f}  ",
    f"no prior anomaly (n={(~has_anomaly).sum()}): cascade MAE {mae_no_anomaly_cascade:.4f} | "
    f"{best_model_name} MAE {mae_no_anomaly_new:.4f}",
    "",
    "## Setup",
    "",
...
results_path = BASE_DIR / "v4_results.md"
