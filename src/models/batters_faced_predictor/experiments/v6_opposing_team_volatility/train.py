"""
batters_faced_predictor experiment v6: opposing team's scoring VOLATILITY
(rolling mean/std/max of runs scored, season-to-date + trailing-5-game) —
follow-up to v5's flat result on scoring LEVEL (win_pct/runs_scored/run_diff).
Run from src/models/batters_faced_predictor/ with:
  python experiments/v6_opposing_team_volatility/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v6 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# v5 tested whether the opposing team's general run-scoring LEVEL (win_pct,
# runs_scored, run_diff — both season-to-date and trailing-5-game) drives
# quicker starter pulls. Result: flat vs. v2 (2.6369 vs. 2.6471), though
# `opp_team_roll_season_runs_scored` did rank #11 of 27 in feature importance
# — a real but modest placement, better than v1's on-base/walk features.
#
# v6 tests a different cut of the same underlying idea: it isn't necessarily
# that a team scores a lot ON AVERAGE that gets a starter in trouble — it's
# whether they're CAPABLE of an explosive/blowout game, i.e. scoring
# volatility, not just level. A team with a high average but low variance
# (steady 5-run games) may be a very different pull risk than one with the
# same average but high variance (mostly 2-3 runs, occasional 10-run
# explosion).
#
#   OPPOSING TEAM SCORING VOLATILITY (NEW):
#   game_context.build_team_scoring_volatility(schedule, window) — new
#   function this pass, TDD'd (tests/hit_predictor/test_game_context.py),
#   point-in-time safe (shift(1)-equivalent, same convention as every other
#   rolling feature in game_context.py). Gives rolling mean/std/max of runs
#   scored per team-game. Joined on the OPPOSING team for this start (same
#   opp_team_id derivation v1/v5 already use), at the same two windows v5
#   used:
#     1. window='season' — season-to-date mean/std/max, primary hypothesis
#        (mean restates v5's own runs_scored/games_n as a convenience; std
#        and max are the new volatility/ceiling signal).
#     2. window=5 — trailing-5-game mean/std/max, secondary recent
#        hot/cold-form-and-volatility signal, same window v5 used.
#
# v5's level features (win_pct, runs_scored, run_diff, both windows) stay in
# the feature list unchanged (additive, not a replacement) — this experiment
# tests whether volatility adds anything BEYOND level, not instead of it.
#
# Evaluated the same way as v2-v5: MAE primary vs v2's own tuned-XGBoost
...
# tuned-XGBoost hyperparameters as v1-v5 (carried over unchanged, per repo
...
# ------------------------- 8. OPPOSING-TEAM SCORING STRENGTH (NEW, v5) ------- #
...
# ------------------------- 9. OPPOSING-TEAM SCORING VOLATILITY (NEW, v6) ----- #
...
_eval("Linear regression (v6)", y_val.to_numpy(), lr.predict(Xval_sc))

print("Training XGBoost (v6, default)...")
import xgboost as xgb
xgb_model = xgb.XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
xgb_model.fit(Xtr, y_train)
_eval("XGBoost (v6, default)", y_val.to_numpy(), xgb_model.predict(Xval))

print("Training XGBoost (v6, tuned)...")
# Hyperparameters and early-stopping setup carried over unchanged from v1-v5.
...
_eval("XGBoost (v6, tuned)", y_val.to_numpy(), xgb_tuned.predict(Xval_core))
...
print(f"\n{'=' * 72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v6 (opposing-team scoring volatility)")
...
if vs_v2 < -0.02:
    print(f"  vs v2's tuned XGBoost (MAE {V2_TUNED_MAE:.4f}): {vs_v2:+.4f} — opposing-team scoring")
    print("  strength adds real signal beyond v2's feature set.")
elif vs_v2 > 0.02:
...
# ── 6d. Established-starter bucket re-check (same convention as v2/v3/v4) ──
...
ax.set_title("Feature importance — XGBoost (v6, tuned)")
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v6_opposing_team_volatility",
            "target": TARGET,
...
# ── 10. Write v6_results.md ───────────────────────────────────────────────────
md_lines = [
    f"# v6 Results — {MODEL_NAME} (opposing-team scoring volatility)",
    "",
...
results_path = BASE_DIR / "v6_results.md"
