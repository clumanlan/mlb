"""
batters_faced_predictor experiment v5: opposing team's general run-scoring
strength (season-to-date + trailing-5-game win_pct/runs_scored/run_diff).
Run from src/models/batters_faced_predictor/ with:
  python experiments/v5_opposing_team_scoring/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v5 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# v3 (cold-start lookback) and v4 (anomaly count) both came back flat trying to
# fix the established-starter early-pull failure mode from
# residual_error_analysis_v2/ — that thread is closed (see ROADMAP.md). v5
# starts a new thread, independent of that failure mode: v1's opposing-lineup
# on-base/walk rate features (team_roll_season_walk_rate/on_base_rate) showed
# only a weak effect (mid-pack feature importance, bottom-third in v1's own
# read). This tests a related but distinct hypothesis — a team's GENERAL
# run-scoring strength, not just its on-base/walk discipline, drives quicker
# pulls for a struggling starter (more traffic of any kind, not just walks,
# means more total batters faced before the same number of outs is recorded).
#
#   OPPOSING TEAM SCORING STRENGTH (NEW):
#   game_context.build_team_win_loss_record(schedule, window) — already
#   exists, already TDD'd, already point-in-time safe (shift(1)-equivalent
#   built into the rolling-sum engine, see tests/hit_predictor/
#   test_game_context.py). Joined on the OPPOSING team for this start (same
#   opp_team_id derivation v1 already uses for team_roll_season_on_base_rate),
#   at two windows:
#     1. window='season' — season-to-date win_pct/runs_scored/run_diff, the
#        "do they score a lot in general" signal and primary hypothesis.
#     2. window=5 — trailing-5-game win_pct/runs_scored/run_diff, a secondary
#        recent hot/cold form signal, same trailing-window pattern v2 already
#        used for the pitcher's own PA trend (window=3).
#
# Zero new production code — both windows are the same already-tested
# function, called twice. Every v1/v2 feature stays in the feature list
# unchanged (additive, not a replacement); v4's anomaly-count feature is
# dropped since that thread is closed and this pass isn't targeting the same
# failure mode — this experiment's feature set is v2's own (baseline + v1 +
# v2) plus the six new opposing-team-scoring columns below.
#
# Evaluated the same way as v2/v3/v4: MAE primary vs v2's own tuned-XGBoost
...
# tuned-XGBoost hyperparameters as v1-v4 (carried over unchanged, per repo
...
_eval("Linear regression (v5)", y_val.to_numpy(), lr.predict(Xval_sc))

print("Training XGBoost (v5, default)...")
import xgboost as xgb
xgb_model = xgb.XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
xgb_model.fit(Xtr, y_train)
_eval("XGBoost (v5, default)", y_val.to_numpy(), xgb_model.predict(Xval))

print("Training XGBoost (v5, tuned)...")
# Hyperparameters and early-stopping setup carried over unchanged from v1-v4.
...
_eval("XGBoost (v5, tuned)", y_val.to_numpy(), xgb_tuned.predict(Xval_core))
...
print(f"\n{'=' * 72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v5 (opposing-team scoring strength)")
...
ax.set_title("Feature importance — XGBoost (v5, tuned)")
...
for name, res in results.items():
    ...
    log_evaluation_to_mlflow(
        ...
        params=params,
        tags={
            "stage": "v5_opposing_team_scoring",
            "target": TARGET,
...
# ── 9. Write v5_results.md ────────────────────────────────────────────────────
md_lines = [
    f"# v5 Results — {MODEL_NAME} (opposing-team scoring strength)",
    "",
...
results_path = BASE_DIR / "v5_results.md"
