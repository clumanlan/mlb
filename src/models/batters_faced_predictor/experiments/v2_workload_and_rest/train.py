"""
batters_faced_predictor experiment v2: trailing-3-start PA/start trend +
pitcher's own rest days + a pitches-thrown workload-density signal.
Run from src/models/batters_faced_predictor/ with:
  python experiments/v2_workload_and_rest/train.py
Requires AWS credentials with read access to s3://mlbdk (us-east-2).
"""
# ---------------------------------------------------------------------------- #
#                                    v2 SUMMARY                                #
# ---------------------------------------------------------------------------- #
# Motivated by experiments/residual_error_analysis/run.py (val MAE 2.7347 for
# v1's tuned XGBoost): the worst over-predictions are established starters
# (cascade weight 0.4-0.86 — high confidence) pulled after 0-2 IP despite clean
# box lines (H/R/ER near 0) — the signature of an in-game injury exit, rain
# delay, or planned short/piggyback outing. Nothing in v1's feature set carries
# recent-start TREND, only season-to-date averages, which react slowly. See
# error_analysis.md for the full worked examples and bucket breakdown.
#
#   1. TRAILING-3-START PA/START TREND: pitcher_last3_start_pa_avg_pa_per_start,
#      via game_context.build_pitcher_start_pa_this_season(pbp, window=3) —
#      already generalized to accept a trailing-N-start window, this pipeline
#      previously only ever called it with window='season'. pa_trend_ratio /
#      pa_trend_direction (recent vs season baseline) are computed as plain
#      arithmetic here, not via interaction_feats.py's regex-driven
#      find_rolling_trend_pairs/build_trend_features — those are built around
#      rolling_stats.py's *_roll_last{N}g_*/*_roll_season_* naming, which
#      game_context.py's pitcher_last{N}_start_pa_*/pitcher_this_season_
#      start_pa_* prefix convention doesn't match. Renaming to fit would touch
#      every existing caller across every sibling model — out of scope.
#   2. PITCHER'S OWN REST DAYS: pitcher_days_since_last_start, via the new
#      game_context.build_pitcher_rest_days(pbp) (TDD'd,
#      tests/hit_predictor/test_game_context.py) — NOT team rest days
#      (pitcher_team_days_since_last_game, already in v1's feature set via
#      build_team_rest_days, stays in the feature list unchanged below) which
#      measures the TEAM's rest (~always 1, teams play daily), not the
#      STARTER's own days since his last start.
#   3. WORKLOAD DENSITY: pitcher_workload_density (= last start's pitch count
#      / days since that start), via the new
#      game_context.build_pitcher_workload_density(pitcher_boxscore, pbp,
#      rest_days) (TDD'd, same test file) — pitches thrown, not batters
#      faced: pitch count is the actual lever managers pull on for a pull
#      decision, and batters-faced recency is already covered by #1.
#      pitcher_workload_density_shrunk = workload_density *
#      (starts_n / (starts_n + k)), k=5.0 (same PA_SHRINKAGE_K used
#      elsewhere in this pipeline) — glue here, not a shared function, same
#      precedent as build_expected_batters_faced's own inline shrinkage
#      weight.
#
# Every v1 feature stays in the feature list unchanged (additive, not a
# replacement). Evaluated the same way as v1: MAE primary vs v1's own tuned-
# XGBoost floor (2.7347), RMSE/Bias/Pearson r secondary, same
# expected_batters_faced_weight-quartile stratification, same bf_gap-quartile
# floor re-check. Same tuned-XGBoost hyperparameters as v1 (carried over
# unchanged, per repo convention — see experiments/xgb_vs_cascade_diagnostic/
# run.py for the overfitting diagnosis that motivated them).
# ---------------------------------------------------------------------------- #
import yaml
...
# ------------------------- 5. TRAILING-3-START PA TREND (NEW) ---------------- #
...
_eval("Linear regression (v2)", y_val.to_numpy(), lr.predict(Xval_sc))

print("Training XGBoost (v2, default)...")
import xgboost as xgb
xgb_model = xgb.XGBRegressor(n_estimators=100, random_state=42, verbosity=0)
xgb_model.fit(Xtr, y_train)
_eval("XGBoost (v2, default)", y_val.to_numpy(), xgb_model.predict(Xval))

print("Training XGBoost (v2, tuned)...")
# Hyperparameters and early-stopping setup carried over unchanged from v1 /
# experiments/xgb_vs_cascade_diagnostic/run.py.
...
_eval("XGBoost (v2, tuned)", y_val.to_numpy(), xgb_tuned.predict(Xval_core))
...
V1_TUNED_MAE = 2.7347  # v1_opposing_traffic_and_rest/train.py's own result, for direct comparison

print(f"\n{'=' * 72}")
print(f"EXPERIMENT RESULTS — {MODEL_NAME} v2 (trailing-3-start PA trend + rest + workload density)")
...
print("-" * 72)
print(f"{'(v1 tuned XGBoost, for reference)':<32} {V1_TUNED_MAE:>8.4f}")
print("=" * 72)
...
vs_v1 = best_model["mae"] - V1_TUNED_MAE
if vs_v1 < -0.02:
    print(f"  vs v1's tuned XGBoost (MAE {V1_TUNED_MAE:.4f}): {vs_v1:+.4f} — the trend/rest/workload")
    print("  features add real signal beyond v1's feature set.")
elif vs_v1 > 0.02:
    print(f"  vs v1's tuned XGBoost (MAE {V1_TUNED_MAE:.4f}): {vs_v1:+.4f} — worse than v1 — check for a")
    print("  wiring bug before concluding the new features don't help.")
else:
    print(f"  vs v1's tuned XGBoost (MAE {V1_TUNED_MAE:.4f}): {vs_v1:+.4f} — flat, no demonstrated")
...
# ── 6d. Over-prediction bucket re-check (the specific failure mode this v2 targets) ──
# error_analysis.md's worst-over-prediction bucket was established starters
# (starts_n >= 11) with a clean box line getting pulled early. Re-run that
# exact slice with v2's best model's predictions to see whether it moved.
print(f"\n{'=' * 72}\nESTABLISHED-STARTER (11+ starts) BUCKET RE-CHECK vs v1\n{'=' * 72}")
...
ax.set_title("Feature importance — XGBoost (v2, tuned)")
...
# ── 9. Write v2_results.md ────────────────────────────────────────────────────
md_lines = [
    f"# v2 Results — {MODEL_NAME} (trailing-3-start PA trend + rest + workload density)",
    "",
...
md_lines += [
    f"| (v1 tuned XGBoost, for reference) | {V1_TUNED_MAE:.4f} | | | | |",
    "",
...
results_path = BASE_DIR / "v2_results.md"
