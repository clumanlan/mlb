...
def run_feature_backfill(season: int, write: bool = True):
    ...
    player_base_features = compute_player_batter_base_features(str(season))

    date_window = player_base_features["game_date"].between(start_date, end_date)
    player_base_features = player_base_features[date_window]
    logger.info(f"Filtered to {len(player_base_features)} rows within season window")

    for date in sorted(player_base_features["game_date"].unique()):
        snapshot = player_base_features[player_base_features["game_date"] == date].copy()
        snapshot["event_timestamp"] = pd.to_datetime(snapshot["game_date"])

        date_str = pd.Timestamp(date).strftime("%Y-%m-%d")
        year = date_str[:4]
        key = f"feast/features/player_batter_base/{year}/{date_str}.parquet"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="player batter base backfill")
