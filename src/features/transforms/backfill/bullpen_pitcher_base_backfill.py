...
from bullpen_pitcher_base import compute_bullpen_features
...
def run_feature_backfill(season: int, write: bool = True):
    ...
    bullpen_base_features = compute_bullpen_features(str(season))

    date_window = bullpen_base_features["game_date"].between(start_date, end_date)
    bullpen_base_features = bullpen_base_features[date_window]
    logger.info(f"Filtered to {len(bullpen_base_features)} rows within season window")

    for date in sorted(bullpen_base_features["game_date"].unique()):
        snapshot = bullpen_base_features[bullpen_base_features["game_date"] == date].copy()
        snapshot["event_timestamp"] = pd.to_datetime(snapshot["game_date"])

        date_str = pd.Timestamp(date).strftime("%Y-%m-%d")
        year = date_str[:4]
        key = f"feast/features/bullpen_pitcher_base/{year}/{date_str}.parquet"
