...
| **Examples tried/considered** | Hit/no-hit per PA (`hit_predictor` v1–v5), run scored | `low_pa` (built), K-prop, BB-prop, starter early exit, bullpen day |
...
| **Strikeout probability (K-prop)** | Pitcher stuff/command vs. batter contact skill — resolved without a ball ever being put in play | Next candidate. Reuses most of `hit_predictor`'s existing infra (rolling stats, TTO/role gating) with a different target column. |
| **Walk probability (BB-prop)** | Pitcher control/zone% vs. batter chase rate — same "three true outcomes" logic as K | Candidate, same infra reuse as K-prop. |
