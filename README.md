...
| **Walk probability (BB-prop)** | Pitcher control/zone% vs. batter chase rate — same "three true outcomes" logic as K | **Built, positive.** Baseline LR beats both naive floors (PR-AUC 0.096 vs. 0.076, ROC-AUC 0.577 vs. 0.501). Scoped to starting pitchers and starting batters only (same population-scoping as K-prop). Zero new feature engineering — reused hit_predictor's existing walk_rate columns directly. |
