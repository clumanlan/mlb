...
Layer 4 — Feature Store       [in progress]  Feast (offline: S3/Athena, online: DynamoDB) — live for n_pa_predictor's 2 features
Layer 5 — Model Training      [in progress]  XGBoost baseline → attention model (see "Model Layer" below)
Layer 6 — Prediction Pipeline [in progress]  daily batch inference (daily_predict), lineup-aware — deployed and live for n_pa_predictor's low_pa model
...
models/{model_name}/{version}.json                 ← model registry, real as of 2026-09-22 (src/shared/model_registry.py)
models/{model_name}/latest.json                    ← always-overwritten alias to the newest version above
predictions/{model_name}/{year}/{date}.parquet     ← daily_predict's output, deployed and live as of 2026-09-23
...
| `daily_feature_materialize` | `src/lambdas/daily_feature_materialize/handler.py` | Calls `FeatureStore(...).materialize_incremental()` against the real Feast registry — the only place `feast` is imported (added 2026-09-17). Container-image Lambda (`package_type=Image`, `architectures=["arm64"]`), not zip+layers — `feast[aws]`'s real dependency closure (~470MB) doesn't fit Lambda's 250MB zip+layers cap. Invoked directly by `daily_feature_create` on success, not on its own schedule. On success, invokes `daily_predict` asynchronously (added 2026-09-22, deployed and confirmed live 2026-09-23 via a real direct invoke — CloudWatch logs show both feature views materialized and `daily_predict` invoked). Deployed and confirmed live via a real S3-triggered smoke test: CloudWatch logs show both feature views materialized, `aws dynamodb scan` confirmed the row-count delta |
| `daily_predict` | `src/lambdas/daily_predict/handler.py` | Batch inference (Production plan step 5): pulls today's confirmed lineups, reads `avg_n_pa_per_game` from Feast's online store (`batting_order` comes straight from the lineup data, no Feast round-trip needed), loads the registered model from `models/{name}/latest.json`, predicts, writes Parquet to `predictions/{name}/{year}/{date}.parquet`. Container-image Lambda, same reason as `daily_feature_materialize` — its own closure is bigger (pulls in `batter_lineup.py`'s full import graph). Invoked by `daily_feature_materialize` on success, not its own schedule. Deployed and confirmed live 2026-09-23 via a real direct invoke of `daily_feature_materialize`: CloudWatch logs show the full chain (materialize → invoke daily_predict → predict → write) completing end to end, real Parquet landed in S3. See `DECISIONS.md`'s 2026-09-22/23 entries |
...
- **[fixed 2026-09-23]** `daily_predict`'s ECR push was blocked by a local Docker Desktop proxy issue (`write tcp ...->192.168.65.1:3128: broken pipe`, repeated 6 times). Resolved by restarting Docker Desktop — the proxy issue cleared, the push succeeded (including the same layer that failed every prior attempt), and `terraform apply` + a manual `aws lambda update-function-code` for `daily_feature_materialize`'s new invoke-chain code both completed. Confirmed live end to end via a real direct invoke: `daily_feature_materialize` materialized both feature views and successfully invoked `daily_predict`, which wrote a real prediction file to S3.
- **`daily_predict`'s cold-start init can exceed Lambda's fixed 10s init budget.** First real invocation logged `INIT_REPORT Init Duration: 9999.24 ms Phase: init Status: timeout` — the image's heavy import closure (feast, xgboost, pandas, scipy, awswrangler) took longer than AWS's non-configurable 10-second container-init cap. Not a failure: AWS retries init within the invocation's own (configurable, 300s) timeout budget, and the invocation still completed correctly (~18-24s total, well under the configured timeout) — but cold starts (first invocation after a period of inactivity) cost noticeably more than warm ones. Not fixed — just documented; revisit if cold-start frequency or latency ever actually matters for this use case (currently: a handful of invocations per day, no user waiting on the response).
